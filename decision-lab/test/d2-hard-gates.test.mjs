...
test("freeze identity is deterministic and validator rejects tampering", async () => {
  ...
  if (additive.mode === "POST_MERGE_ACTIVE_CHAIN") {
    ...
    for (const path of ["decision-lab/src/admin-data-additive.mjs", "decision-lab/src/d2-cli.mjs", "decision-lab/src/recertification-consumer.mjs", "decision-lab/src/recertification-verify.mjs"]) await writeFile(join(baselineRoot, path), await readFile(join(root, path)));
