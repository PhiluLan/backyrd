...
test("freeze identity is deterministic and validator rejects tampering", async () => {
  ...
  if (additive.mode === "POST_MERGE_ACTIVE_CHAIN") {
    ...
    for (const path of ["decision-lab/src/admin-data-additive.mjs", "decision-lab/src/mobile-storage-atomic.mjs", "decision-lab/src/d2-cli.mjs", "decision-lab/src/recertification-consumer.mjs", "decision-lab/src/recertification-verify.mjs"]) await writeFile(join(baselineRoot, path), await readFile(join(root, path)));
    execFileSync("git", ["config", "user.email", "fixture@example.invalid"], { cwd: baselineRoot }); execFileSync("git", ["config", "user.name", "Fixture"], { cwd: baselineRoot }); execFileSync("git", ["add", "."], { cwd: baselineRoot }); execFileSync("git", ["commit", "--allow-empty", "-m", "install consumer under test"], { cwd: baselineRoot });
