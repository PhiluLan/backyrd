...
const sourceUrl = new URL("../../supabase/functions/decision-v13/index.ts", import.meta.url);

export async function loadCanonicalDecisionHandler({ env, embeddingMode }) {
  ...
  return { handler, sourceHash: createHash("sha256").update(source).digest("hex"), getTrace: () => structuredClone(globalThis.__backyrdDecisionLabTrace ?? null), restore: () => { globalThis.fetch = originalFetch; delete globalThis.Deno; delete globalThis.__backyrdDecisionLabHandler; delete globalThis.__backyrdDecisionLabTrace; } };
}
