...
const finalExperiment = option("experiment") ?? "H1_DETERMINISTIC_STRUCTURED";
if (!configOption || !outputOption) throw new Error("--config and --output are required");

const preflight = await d31Preflight();
if (preflight.status !== "PASS") throw new Error(`Wave 2.5 preflight failed: ${preflight.reasons.join(",")}`);
...
if (!["FULL_FIDELITY", "FAST_SIMULATION"].includes(config.embeddingMode)) throw new Error("Unsupported Wave 2.5 embedding mode");
...
const run = await runD3AWorld({
  ...
  engine: {
    sourceUrl, expectedSourceHash, baselineId: "backyrd-wave2.5-retrieval-relevance-v1", goldenOnly: true,
    ...
    retrievalShortlisting: true, retrievalShortlistingLimits: { union: retrievalContract.thresholds.candidatePoolMeanMaximum, shortlist: retrievalContract.promotionK },
    retrievalRelevance: true, retrievalRelevanceLimits: { shortlist: retrievalContract.promotionK }, retrievalRelevanceExperiment: finalExperiment,
  },
});
const result = {
  version: "wave2.5-retrieval-relevance-arm-v1",
...
const diagnosticRecords = result.records.filter((record) => record.split === "DEVELOPMENT");
const diagnosticSummary = (experiment) => summarizeRetrievalScenarios(diagnosticRecords.map((record) => {
  const candidates = record.retrievalRelevance.experiments[experiment];
  return {
    metrics: retrievalScenarioMetrics({ candidateIds: candidates.map((row) => row.spot_id), eligibleUtilityById: record.retrievalContractEvidence.eligibleUtilityById, relevanceThreshold: retrievalContract.relevanceThreshold, k: retrievalContract.promotionK }),
...
process.stdout.write(`${JSON.stringify({ status: "PASS", seed: config.seed, mode: config.embeddingMode, scenarios: result.records.length, diagnosticsRestrictedTo: "DEVELOPMENT", developmentScenarios: diagnosticRecords.length, hardViolations: result.records.filter((row) => !row.hardConstraintResult.pass).length, experiments: Object.fromEntries(experiments.map((experiment) => [experiment, diagnosticSummary(experiment)])) }, null, 2)}\n`);
