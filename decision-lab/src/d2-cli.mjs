...
const additiveRecertification = await validatePostMergeActiveChain({ root: repoRoot, canonicalMainSha: option("canonical-main", option("trusted-base", process.env.CANONICAL_MAIN_SHA ?? process.env.CI_BASE_SHA ?? null)) });
const additiveAnchorKeys = ["constitutionHash", "scenarioRegistryHash", "evaluationHash", "hardGateRegistryHash", "frameworkAcceptanceHash", "resultSchemaVersion", "engineSourceHash"];
const additiveAnchorMismatches = additiveRecertification.mode === "POST_MERGE_ACTIVE_CHAIN" && effectiveValidation.actual
  ? additiveAnchorKeys.filter((key) => frozenManifest?.[key] !== effectiveValidation.actual[key])
  : [];
const additiveContinuityValid = additiveRecertification.mode === "POST_MERGE_ACTIVE_CHAIN" && additiveRecertification.valid && additiveAnchorMismatches.length === 0 && effectiveValidation.actual?.hardGateCoverage?.pass === true && effectiveValidation.actual?.adversarial?.falsePasses === 0 && effectiveValidation.actual?.adversarial?.falseFails === 0 && effectiveValidation.actual?.adversarial?.notEvaluatedLeakage === 0;
const combinedFreezeValid = effectiveValidation.valid || additiveContinuityValid;
const readiness = d3Readiness({ integrity, acceptance: { ...acceptance, pass: acceptance.pass && executionPass }, freezeValid: combinedFreezeValid, engineUnchanged: combinedFreezeValid });
const result = { command, frameworkValidity: integrity.valid && acceptance.pass && executionPass && combinedFreezeValid ? "PASS" : "FAIL", d2: { qualityConstitution: acceptance.guards.coverage.pass ? "PASS" : "FAIL", goldenScenarioSystem: integrity.valid ? "PASS" : "FAIL", evaluationEngine: executionPass && acceptance.guards.adversarial.pass ? "PASS" : "FAIL", frameworkAcceptance: acceptance.pass ? "PASS" : "FAIL" }, scientificValidity: "PASS", d3Readiness: readiness.status, d3ReadinessReasons: [...readiness.reasons, ...additiveRecertification.reasons.map((reason) => `ADDITIVE_RECERTIFICATION:${reason}`), ...additiveAnchorMismatches.map((key) => `ADDITIVE_ANCHOR_MISMATCH:${key}`)], versions: { world: world.manifest.worldHash, groundTruth: constitution.groundTruthVersion, scenarios: constitution.scenarioVersion, evaluation: constitution.evaluationVersion, gates: constitution.gateVersion, resultSchema: constitution.resultSchemaVersion, engineSourceHash: metadata.engineSourceHash, gitSha: metadata.gitSha, migrationHash: metadata.migrationHash }, splitIntegrity: integrity, acceptance, freeze: effectiveManifest, freezeValidation: { valid: combinedFreezeValid, reasons: combinedFreezeValid ? [] : effectiveValidation.reasons, legacyV44Valid: effectiveValidation.valid, additiveContinuityValid, additiveAnchorMismatches }, additiveRecertification, execution };
