...
const additiveRecertification = await validateActiveAdditiveRecertification({ root: repoRoot, trustedBaseSha: option("trusted-base", process.env.CI_BASE_SHA ?? null) });
...
const additiveAnchorMismatches = additiveRecertification.mode === "ADDITIVE_CHAIN" && effectiveValidation.actual
  ? additiveAnchorKeys.filter((key) => frozenManifest?.[key] !== effectiveValidation.actual[key])
  : [];
const additiveContinuityValid = additiveRecertification.mode === "ADDITIVE_CHAIN" && additiveRecertification.valid && additiveAnchorMismatches.length === 0 && effectiveValidation.actual?.hardGateCoverage?.pass === true && effectiveValidation.actual?.adversarial?.falsePasses === 0 && effectiveValidation.actual?.adversarial?.falseFails === 0 && effectiveValidation.actual?.adversarial?.notEvaluatedLeakage === 0;
