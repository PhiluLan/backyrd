...
if (preflight.status !== "PASS") throw new Error(`Wave 2.4 preflight failed: ${preflight.reasons.join(",")}`);
...
if (!["FULL_FIDELITY", "FAST_SIMULATION"].includes(config.embeddingMode)) throw new Error("Unsupported Wave 2.4 embedding mode");
...
const run = await runD3AWorld({
  ...
  engine: {
    sourceUrl, expectedSourceHash, baselineId: "backyrd-wave2.4-retrieval-shortlisting-v1", goldenOnly: true,
...
const result = {
  version: "wave2.4-retrieval-shortlisting-arm-v1",
...
const diagnosticSummary = (experiment) => summarizeRetrievalScenarios(result.records.map((record) => {
  const candidates = record.retrievalShortlisting.experiments[experiment];
  return { metrics: retrievalScenarioMetrics({ candidateIds: candidates.map((row) => row.spot_id), eligibleUtilityById: record.retrievalContractEvidence.eligibleUtilityById, relevanceThreshold: retrievalContract.relevanceThreshold, k: retrievalContract.promotionK }), latencyMs: experiment === "H0_WAVE2_3" ? record.retrievalRebuild.latencyMs : record.retrievalShortlisting.latencyMs, resultCount: record.finalTopK.length, hardViolation: !record.hardConstraintResult.pass };
}));
process.stdout.write(`${JSON.stringify({ status: "PASS", seed: config.seed, mode: config.embeddingMode, scenarios: result.records.length, hardViolations: result.records.filter((row) => !row.hardConstraintResult.pass).length, experiments: Object.fromEntries(experiments.map((experiment) => [experiment, diagnosticSummary(experiment)])) }, null, 2)}\n`);
