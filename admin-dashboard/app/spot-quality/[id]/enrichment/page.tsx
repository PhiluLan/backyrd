...
type EnrichmentResponse={ok:boolean;error?:string;preview:Preview;applied:string[]};
...
export default function GoogleEnrichmentPage(){
  ...
  async function invoke(body:Record<string,unknown>):Promise<EnrichmentResponse>{
    const {data,error}=await supabase.functions.invoke<EnrichmentResponse>("spot-google-enrichment",{body});
  ...
  setSelected(
    ...
    catch(e:unknown){setError(message(e,"Google-Daten konnten nicht geladen werden."))}finally{setLoading(false)}
  }
  function toggle(field:string){setSelected(v=>v.includes(field)?v.filter(x=>x!==field):[...v,field])}
  async function apply(){if(!id||!selected.length)return;setSaving(true);setError("");setSuccess("");try{const data=await invoke({action:"apply",spotId:id,fields:selected});setSuccess(`${data.applied.length} Felder übernommen.`);await load()}catch(e:unknown){setError(message(e,"Google-Daten konnten nicht übernommen werden."))}finally{setSaving(false)}}
