...
function proposalValue(value: unknown): string {
  ...
  try { return JSON.stringify(value); } catch { return "Nicht darstellbarer strukturierter Wert"; }
}
...
export function GoldAuthoringPanel({ spotId, refreshToken = 0 }: { spotId: string; refreshToken?: number }) {
  ...
  return <section className="spot-editor-section hsi-v2">
    ...
    {profile.proposals.some((proposal) => ["PENDING", "CONFLICT", "STALE"].includes(proposal.status)) && <section className="hsi-proposals"><header><div><span>Vorschläge</span><h3>Backyrd hat dazu Angaben gefunden</h3><p>Ein Vorschlag ist nie vorausgewählte Wahrheit. Prüfe Wert, Geltungsbereich und Quelle bewusst.</p></div></header><div>{profile.proposals.filter((proposal) => ["PENDING", "CONFLICT", "STALE"].includes(proposal.status)).map((proposal) => {
      const proposalScope = proposal.evidence_scope ?? proposal.research_evidence_scope;
      const proposalSource = sources.get(proposal.source_id);
      return <article key={proposal.id} data-proposal-id={proposal.id}><div className="hsi-proposal-evidence"><strong>{profile.questions.find((question) => question.canonical_field_key === proposal.field_key)?.label_de ?? proposal.field_key}</strong><code>{proposalValue(proposal.proposed_value)}</code><span>Proposal {proposal.id} · Feld {proposal.field_key}</span><span>{proposalScope === "SPOT" ? "Gilt allgemein für den Ort" : `Scope ${proposalScope ?? "UNKNOWN"} · keine automatische Spot-Wahrheit`}{proposal.research_entity_scope ? ` · Entity ${proposal.research_entity_scope}` : ""}{proposal.research_durability ? ` · ${proposal.research_durability}` : ""}{proposal.research_scope_resolution ? ` · ${proposal.research_scope_resolution}` : ""}</span>{proposalSource?.source_url && <a href={proposalSource.source_url} target="_blank" rel="noreferrer">{proposalSource.title || proposalSource.source_url} ↗</a>}{proposal.evidence_excerpt && <blockquote>{proposal.evidence_excerpt}</blockquote>}</div><div><button type="button" aria-label={`Proposal ${proposal.id} ablehnen`} disabled={busy !== null} onClick={() => void reviewProposal(proposal.id, "REJECT")}>Ablehnen</button><button type="button" aria-label={`Proposal ${proposal.id} bestätigen`} disabled={busy !== null || proposalScope !== "SPOT"} onClick={() => void reviewProposal(proposal.id, "ACCEPT")}>{busy === proposal.id ? "Wird geprüft …" : "Bestätigen"}</button></div></article>;
    })}</div></section>}
