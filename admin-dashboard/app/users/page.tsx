...
export default function UsersPage(){
 ...
 <section className="bi-card admin-responsiveTable"><div className="bi-tableToolbar"><input className="bi-input" placeholder="Name, E-Mail, Stadt oder UUID suchen …" value={search} onChange={e=>setSearch(e.target.value)}/><span>{rows.length} Nutzer</span></div><div className="bi-tableWrap"><table className="bi-table"><thead><tr><th>User</th><th>Status</th><th>Letzte Aktivität</th><th>Sessions</th><th>Screens</th><th>Spots</th><th>Decisions</th><th>Reviews</th><th></th></tr></thead><tbody>{rows.map(u=><tr key={u.user_id}><td data-label="Nutzer"><div className="bi-person">{u.avatar_url?<img src={u.avatar_url} alt=""/>:<i/>}<div><strong>{u.display_name||u.username||u.email||"Backyrd User"}</strong><small>{u.email||u.user_id}</small></div></div></td><td data-label="Status"><span className={`bi-badge ${u.activated?"success":"muted"}`}>{u.activated?"Aktiviert":"Nicht aktiviert"}</span></td><td data-label="Letzte Aktivität">{dateTime(u.last_active_at)}</td><td data-label="Sessions">{number(u.sessions)}</td><td data-label="Screens">{number(u.screen_views)}</td><td data-label="Spots">{number(u.spot_opens)}</td><td data-label="Decisions">{number(u.decisions)}</td><td data-label="Reviews">{number(u.reviews)}</td><td data-label="Aktion"><Link className="bi-action" href={`/users/${u.user_id}?from=${encodeURIComponent(rangeFor(preset).from)}&to=${encodeURIComponent(rangeFor(preset).to)}`}>Details öffnen</Link></td></tr>)}</tbody></table></div></section></>}
 </div>;
}
