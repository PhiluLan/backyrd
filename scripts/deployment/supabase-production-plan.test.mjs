...
test("all migrations since the shipped baseline remain pending across later evidence commits", () => {
  const f = fixture();
  const first = "supabase/migrations/20260908045502_first.sql";
  const second = "supabase/migrations/20260908050000_second.sql";
  write(f.repo, first, "select 1;\n"); commit(f.repo, "first Product merge");
  write(f.repo, "docs/evidence.md", "later evidence\n"); commit(f.repo, "later evidence");
  write(f.repo, second, "select 2;\n"); const head = commit(f.repo, "second Product merge");
