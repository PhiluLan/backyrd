...
test("Founder-authorized pre-applied migrations import exact bytes without a second runtime deploy", () => { const f=fixture(); const migration="supabase/migrations/20260905193445_events_v1.sql"; write(f.repo,migration,"select 1;\n"); const digest=execFileSync("sha256sum",[join(f.repo,migration)],{encoding:"utf8"}).split(/\s+/)[0]; write(f.repo,"supabase/production/preapplied-migration-import.json",`${JSON.stringify({version:"backyrd-preapplied-migration-import-v1",projectRef:"hjgcrrzfjchzqoegcywn",canonicalBaseSha:f.base,remoteState:"REMOTE_UP_TO_DATE",schemaVerification:"PASS",migrations:[{path:migration,sha256:digest}]},null,2)}\n`); const result=plan(f,commit(f.repo,"pre-applied import")); assert.deepEqual(result.migrations,[{path:migration,sha256:digest}]); assert.deepEqual(result.pendingMigrations,[]); assert.deepEqual(result.preappliedMigrationImport.migrations,[{path:migration,sha256:digest}]); assert.equal(result.runtimeDeploymentRequired,false); });
test("Pre-applied migration import with a different base or bytes fails closed", () => { const f=fixture(); const migration="supabase/migrations/20260905193445_events_v1.sql"; write(f.repo,migration,"select 1;\n"); write(f.repo,"supabase/production/preapplied-migration-import.json",`${JSON.stringify({version:"backyrd-preapplied-migration-import-v1",projectRef:"hjgcrrzfjchzqoegcywn",canonicalBaseSha:"0".repeat(40),remoteState:"REMOTE_UP_TO_DATE",schemaVerification:"PASS",migrations:[{path:migration,sha256:"0".repeat(64)}]},null,2)}\n`); const head=commit(f.repo,"invalid pre-applied import"); assert.throws(()=>plan(f,head),/preapplied_migration_base_mismatch/); });
