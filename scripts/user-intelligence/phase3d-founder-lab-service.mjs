...
export class FounderLabService {
  ...
  #evaluation(user) {
    ...
    const items = []; const decisionTaskBindings = []; const byId = new Map();
    for (const action of user.actions) {
    ...
    const report = buildPhase3DFounderLabReport(input, createPhase3DCanonicalReleaseTrust(), trust, createPhase3DRepositoryTrust(), undefined, { decisionTaskBindings });
    ...
    return { items, decisionTaskBindings, trust, input, report, state };
  }
  view(message = null, operation = null) {
    ...
    const timeline = selected?.actions.map((action) => ({ actionId: action.actionId, occurredAt: action.occurredAt, journeyId: action.journeyId, decisionId: action.decisionId, experienceId: action.experienceId, spotId: action.spotId, decisionTaskId: action.context.intentId, contextHash: contextHash(action.context), contextLabel: `${action.context.dayPhase} · ${action.context.companyType} · ${action.context.timeBudget}`, action: action.description, authorityStatus: "SYNTHETIC_SERVER_BOUND", status: corrected.has(action.actionId) ? "CORRECTED" : action.active ? "ACTIVE" : "INACTIVE", independenceUnit: action.active ? action.journeyId : null, target: action.conceptIds.length ? action.conceptIds.join(", ") : action.spotId, direction: action.eventType === "EXPLICIT_SATISFACTION" ? "POSITIVE" : action.eventType === "EXPLICIT_DISSATISFACTION" || ["QUICK_SKIP", "SPOT_NOT_FIT"].includes(action.eventType) ? "NEGATIVE_OR_WEAK" : "NEUTRAL", withheld: action.attentionOnly || action.learningSuppressedReason !== null || ["SEARCH", "QUICK_SKIP", "SPOT_NOT_FIT"].includes(action.eventType), eventType: action.eventType })) ?? [];
