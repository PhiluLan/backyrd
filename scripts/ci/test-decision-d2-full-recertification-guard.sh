...
# The complete current v12 re-certification must be admitted against its base.
...
(cd "$identity_case" && node -e 'const fs=require("node:fs");const p="decision-lab/config/decision-v13-production-recertification-v12.json";const v=JSON.parse(fs.readFileSync(p,"utf8"));v.production.activeVersion=122;fs.writeFileSync(p,JSON.stringify(v,null,2)+"\n")')
git -C "$identity_case" add decision-lab/config/decision-v13-production-recertification-v12.json
...
echo "D2 full re-certification guard regression: unchanged PASS; Engine drift FAIL; new source FAIL; Production identity drift FAIL; complete v12 re-certification PASS"
