...
# The complete current v41 re-certification must be admitted against its base.
...
(cd "$identity_case" && node -e 'const fs=require("node:fs");const p="decision-lab/config/decision-v13-production-recertification-v41.json";const v=JSON.parse(fs.readFileSync(p,"utf8"));v.production.activeVersion=125;fs.writeFileSync(p,JSON.stringify(v,null,2)+"\n")')
git -C "$identity_case" add decision-lab/config/decision-v13-production-recertification-v41.json
...
(cd "$future_case" && node -e 'const fs=require("node:fs");const p="decision-lab/config/decision-v13-production-recertification-v41.json";const v=JSON.parse(fs.readFileSync(p,"utf8"));v.version="decision-v13-production-recertification-v42";fs.writeFileSync(p,JSON.stringify(v,null,2)+"\n")')
git -C "$future_case" add decision-lab/config/decision-v13-production-recertification-v41.json
...
echo "D2 full re-certification guard regression: unchanged PASS; Engine drift FAIL; new source FAIL; Production identity drift FAIL; unknown future recertification FAIL; complete v41 re-certification PASS"
