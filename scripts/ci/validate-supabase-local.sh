...
assert_count 4 'canonical cron jobs' \
  "select count(*) from cron.job where active and (command like '%/functions/v1/generate-spot-embeddings%' or command like '%/functions/v1/safety-text-worker%' or command like '%/functions/v1/safety-image-worker%' or jobname = 'backyrd-account-trust-identity-daily');"
