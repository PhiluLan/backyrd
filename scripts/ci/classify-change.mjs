...
export function classifyChange({ root, context, policy }) {
  ...
  const flags = {
    ...
    database: changedFiles.some((path) => startsWithAny(path, policy.databasePrefixes)),
  ...
  const classes = [
    ...
    ...(flags.database ? [flags.authorizationBoundary ? "authorization-boundary" : "database-additive"] : []),
  ...
  return {
    ...
    requiredGates: unique([
      ...
      ...(flags.deliveryControl || flags.releaseEvidence || flags.database ? ["delivery-contract"] : []),
    ]),
