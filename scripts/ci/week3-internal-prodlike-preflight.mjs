...
export function verifyWeek3CandidateIdentities(root, manifest) {
  for (const candidate of manifest.domainCandidates) {
    ...
    requireValue(git(root, ["rev-parse", `${candidate.mergeSha}^{tree}`]) === candidate.mergeTreeSha, `week3_candidate_merge_tree_mismatch:${candidate.track}`);
    requireValue(git(root, ["show", "-s", "--format=%P", candidate.mergeSha]).split(" ").filter(Boolean).join(",") === candidate.mergeParents.join(","), `week3_candidate_merge_parents_mismatch:${candidate.track}`);
    requireValue(git(root, ["merge-base", "--is-ancestor", candidate.mergeSha, manifest.canonicalBaseSha]) === "", `week3_candidate_merge_not_in_canonical_main:${candidate.track}`);
...
export function runWeek3Preflight({ root = ROOT, mode, baseSha: requestedBase, headSha: requestedHead, checkoutSha: requestedCheckout = "HEAD", canonicalMainRef = "origin/main", final = false }) {
  ...
  requireValue(JSON.stringify(documents.evidence.orderedHeads) === JSON.stringify(reconstruction.orderedHeads) && JSON.stringify(documents.evidence.domainLineage) === JSON.stringify(reconstruction.domainLineage) && JSON.stringify(documents.evidence.steps) === JSON.stringify(reconstruction.steps) && JSON.stringify(documents.evidence.overlaps) === JSON.stringify(reconstruction.overlaps) && JSON.stringify(documents.evidence.conflictResolutions) === JSON.stringify(reconstruction.conflictResolutions), "week3_rehearsal_detail_mismatch");
  ...
  requireValue(productionPlan.pendingMigrations.length === 9 && JSON.stringify(productionPlan.pendingMigrations.map(({ path }) => path)) === JSON.stringify(documents.manifest.productionPlan.expectedPendingMigrations) && productionPlan.pendingMigrations.every(({ path }) => /world_knowledge|world_founder/.test(path)), "week3_pending_migrations_invalid");
  ...
  return { contractVersion: "backyrd.week3-preflight-result@1.0", status: "GREEN", executionAuthorized: false, identity, candidate: { baseSha, headSha, treeSha: git(root, ["rev-parse", `${headSha}^{tree}`]) }, canonicalMainSha, reconstruction, artifact: { artifactHash: artifactExecution.artifact.artifactHash, sourceSetHash: artifactExecution.artifact.sourceSetHash, manifestHash: sealedArtifactFileHash, verifiedTracks: artifactExecution.verifiedTracks }, decisionAuthority, allowlist: { realMemberCount: 0, syntheticEnvelopeHash: envelope.envelopeHash, releaseHash }, killSwitchRehearsal: { off: full.phases.off, testOn: full.phases.testOn, emergencyOff: emergency.phases.emergencyOff, repeatedOff: repeatedOff.phases.off, recoveryMilliseconds: emergency.recoveryMilliseconds, qualityThresholdDeclared: false }, reportHash, replayHash: reportHash, supabaseCompatibility: { ...supabaseCompatibility, ...securityDefiner }, productionPlan: { planHash, pendingMigrationCount: 9, pendingMigrations: productionPlan.pendingMigrations, deployFunctions: [], authDeploy: false, runtimeActivation: false, executionAuthorized: false }, postDeployEvidenceStatus: documents.postDeployEvidence.status, finalMode: final };
}
