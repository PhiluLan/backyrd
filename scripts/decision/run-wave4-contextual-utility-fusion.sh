...
lab_root="$(mktemp -d "${TMPDIR:-/tmp}/backyrd-wave4.XXXXXX")"
project_id="backyrd-wave4-contextual-utility"
started=false

cleanup() {
  if test "$started" = true; then supabase stop --project-id "$project_id" --no-backup >/dev/null 2>&1 || true; fi
  case "$lab_root" in
    "${TMPDIR:-/tmp}"/backyrd-wave4.*|/tmp/backyrd-wave4.*) rm -rf "$lab_root" ;;
    *) printf 'Refusing unexpected cleanup path: %s\n' "$lab_root" >&2 ;;
  esac
}
trap cleanup EXIT

node "$repo_root/decision-lab/src/wave4-freeze.mjs" validate >/dev/null
...
sed -i.bak -e 's/port = 54321/port = 59721/' -e 's/port = 54322/port = 59722/' -e 's/shadow_port = 54320/shadow_port = 59720/' -e 's/port = 54329/port = 59729/' -e 's/port = 54323/port = 59723/' -e 's/port = 54324/port = 59724/' -e 's/port = 54327/port = 59727/' "$lab_root/supabase/config.toml"
...
for seed in backyrd-d1-basel-v1-2026 backyrd-d1-basel-v1-2026-2 backyrd-d1-basel-v1-2026-3; do
  ...
  node "$repo_root/decision-lab/src/wave4-world-cli.mjs" --config "$config" --output "$lab_root/output/$seed.json"
done

supabase stop --project-id "$project_id" --no-backup >/dev/null 2>&1
started=false
node "$repo_root/decision-lab/src/wave4-aggregate.mjs" --input "$lab_root/output" --output "$repo_root/decision-lab/baselines/wave4-contextual-utility-fusion-v1.json"
printf 'Wave 4 official run completed; disposable environment destroyed.\n'
