...
lab_root="$(mktemp -d "${TMPDIR:-/tmp}/backyrd-wave2-5.XXXXXX")"
project_id="backyrd-wave2-5-relevance"
started=false

cleanup() {
  if test "$started" = true; then supabase stop --project-id "$project_id" --no-backup >/dev/null 2>&1 || true; fi
  case "$lab_root" in
    "${TMPDIR:-/tmp}"/backyrd-wave2-5.*|/tmp/backyrd-wave2-5.*) rm -rf "$lab_root" ;;
    *) printf 'Refusing unexpected cleanup path: %s\n' "$lab_root" >&2 ;;
  esac
}
trap cleanup EXIT

mode="${WAVE2_5_MODE:-FULL_FIDELITY}"
seeds="${WAVE2_5_SEEDS:-backyrd-d1-basel-v1-2026 backyrd-d1-basel-v1-2026-2 backyrd-d1-basel-v1-2026-3}"
experiment="${WAVE2_5_EXPERIMENT:-H1_DETERMINISTIC_STRUCTURED}"
...
sed -i.bak -e 's/port = 54321/port = 59221/' -e 's/port = 54322/port = 59222/' -e 's/shadow_port = 54320/shadow_port = 59220/' -e 's/port = 54329/port = 59229/' -e 's/port = 54323/port = 59223/' -e 's/port = 54324/port = 59224/' -e 's/port = 54327/port = 59227/' "$lab_root/supabase/config.toml"
...
for seed in $seeds; do
  ...
  node "$repo_root/decision-lab/src/wave2.5-world-cli.mjs" --config "$config" --output "$lab_root/output/$seed.json" --experiment "$experiment"
done

if test -n "${WAVE2_5_DIAGNOSTIC_ARTIFACT_DIR:-}"; then
  case "$WAVE2_5_DIAGNOSTIC_ARTIFACT_DIR" in
    /tmp/*|"${TMPDIR:-/tmp}"/*) mkdir -p "$WAVE2_5_DIAGNOSTIC_ARTIFACT_DIR"; cp "$lab_root"/output/*.json "$WAVE2_5_DIAGNOSTIC_ARTIFACT_DIR"/ ;;
    *) printf 'Refusing non-temporary diagnostic artifact directory: %s\n' "$WAVE2_5_DIAGNOSTIC_ARTIFACT_DIR" >&2; exit 1 ;;
  esac
fi
...
if test "$mode" = FULL_FIDELITY && test "$seeds" = "backyrd-d1-basel-v1-2026 backyrd-d1-basel-v1-2026-2 backyrd-d1-basel-v1-2026-3"; then
  node "$repo_root/decision-lab/src/wave2.5-aggregate.mjs" --input "$lab_root/output" --full-fidelity "$lab_root/full-fidelity" --query-cache "$lab_root/full-fidelity/query-cache.json" --output "$repo_root/decision-lab/baselines/wave2.5-retrieval-relevance-v1.json"
  printf 'Wave 2.5 FULL_FIDELITY comparison completed; disposable environment destroyed.\n'
else
  printf 'Wave 2.5 diagnostic subset completed; no promotable baseline was written.\n'
fi
