...
test("Founder Decision Lab is loopback-only, no-store, and performs resolve/evaluate/replay", async () => {
  ...
  try {
    ...
    const artifact = makeFounderCohortHandoff(); const preview = await fetch(`${url}/api/cohort/preview`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ artifact }) }).then((response) => response.json()); assert.equal(preview.data.spotCount, 5);
    ...
    const founderResult = await fetch(`${url}/api/evaluate`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) }).then((response) => response.json()); assert.equal(founderResult.data.worldCohort.source, "FOUNDER_WORLD_COHORT"); assert.equal(founderResult.data.candidates.length, 5);
