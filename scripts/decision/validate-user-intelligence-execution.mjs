...
import { buildUserIntelligenceReadOnly, runQueueOnce, drainQueue, SupabaseUserIntelligenceRepository } from "../../packages/user-intelligence-runtime/src/index.mjs";
...
try{
 ...
 must((await service.from("spots").insert([{id:spots.a,name:"Execution A",lat:47.55,lng:7.59,status:"approved",data_origin:"FIXTURE"},{id:spots.b,name:"Execution B",lat:47.56,lng:7.60,status:"approved",data_origin:"FIXTURE"},{id:spots.c,name:"Execution C",lat:47.57,lng:7.61,status:"approved",data_origin:"FIXTURE"}])).error,"spots");
 ...
 // Missing-N4 honesty uses a valid Product mood. Unsupported moods are
 // rejected at ingestion and are covered by the canonical registry tests.
 must((await service.from("reviews").insert({id:reviews.c,user_id:users.a.id,spot_id:spots.c,product_evidence_origin:"smart_review_v1",mood_a:"gemütlich",text:"War gut."})).error,"review_c");
 ...
 const directSource=await repository.readCanonicalSources(users.a.id),directResult=buildUserIntelligenceReadOnly({userId:users.a.id,source:directSource}),directInput=directResult.input,directCard=directResult.validated.card;if(directCard.userCardHash!==finalSnapshot.snapshot_hash)throw new Error(`direct_shared_runtime_persisted_snapshot_mismatch:${JSON.stringify({persisted:finalSnapshot.snapshot_hash,direct:directCard.userCardHash,persistedWatermark:finalSnapshot.source_watermark,directWatermark:directSource.watermark,inputEvents:directInput.length})}`);
 ...
 try{await service.from("spots").delete().in("id",Object.values(spots))}catch{}
}
