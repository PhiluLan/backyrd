...
try{
 ...
 const{data:foreignCards,error:foreignCardError}=await clientA.from("backyrd_user_intelligence_snapshots_v2").select("snapshot_id").eq("user_id",users.b.id);output.security.foreignCardHidden=!foreignCardError&&foreignCards.length===0;
 ...
 if(Object.values(output.security).some((value)=>!value))throw new Error("cross_user_security_failed");

 // Active-user burst benchmark through the real queue and persistence path.
 const burstStart=performance.now();await Promise.all(Array.from({length:100},(_,index)=>ingest(users.a.id,`burst:${suffix}:${index}`,"candidate_exposed",spots.a,`burst-${suffix}-${index}`,`burst-${suffix}-${index}`)));const burstRun=await runQueueOnce({repository});output.performance.hundredEventBurstMs=Number((performance.now()-burstStart).toFixed(3));output.performance.hundredEventBurstStatus=burstRun.status;const postBurstHash=(await snapshot()).snapshot_hash;
 const rebuildStart=performance.now();must((await service.rpc("backyrd_enqueue_user_intelligence_rebuild_v1",{p_user_id:users.a.id,p_reason:"PERFORMANCE_FULL_REBUILD"})).error,"enqueue_perf_rebuild");const fullRebuild=await runQueueOnce({repository});output.performance.fullRebuildMs=Number((performance.now()-rebuildStart).toFixed(3));output.performance.fullRebuildStatus=fullRebuild.status;const finalSnapshot=await snapshot();if(postBurstHash!==finalSnapshot.snapshot_hash)throw new Error("progressive_full_rebuild_mismatch");
