...
lab_root="$(mktemp -d "${TMPDIR:-/tmp}/backyrd-wave2-1.XXXXXX")"
project_id="backyrd-wave2-1-comparison"
started=false

cleanup() {
  if test "$started" = true; then supabase stop --project-id "$project_id" --no-backup >/dev/null 2>&1 || true; fi
  case "$lab_root" in
    "${TMPDIR:-/tmp}"/backyrd-wave2-1.*|/tmp/backyrd-wave2-1.*) rm -rf "$lab_root" ;;
    *) printf 'Refusing unexpected cleanup path: %s\n' "$lab_root" >&2 ;;
  esac
}
trap cleanup EXIT

modes="${WAVE2_1_MODES:-FAST_SIMULATION FULL_FIDELITY}"
seeds="${WAVE2_1_SEEDS:-backyrd-d1-basel-v1-2026 backyrd-d1-basel-v1-2026-2 backyrd-d1-basel-v1-2026-3}"
...
sed -i.bak -e 's/port = 54321/port = 58721/' -e 's/port = 54322/port = 58722/' -e 's/shadow_port = 54320/shadow_port = 58720/' -e 's/port = 54329/port = 58729/' -e 's/port = 54323/port = 58723/' -e 's/port = 54324/port = 58724/' -e 's/port = 54327/port = 58727/' "$lab_root/supabase/config.toml"
...
for mode in $modes; do
  ...
  for seed in $seeds; do
    ...
    if test "$mode" = FULL_FIDELITY; then
      node "$repo_root/decision-lab/src/wave2-full-fidelity-embeddings.mjs" estimate --world "$world_root/world.json" --query-token-allowance 18000
      full_sql="$lab_root/full-fidelity/${seed}.sql"
      node "$repo_root/decision-lab/src/wave2-full-fidelity-embeddings.mjs" generate --world "$world_root/world.json" --query-token-allowance 18000 --max-usd 1 --output-sql "$full_sql" --output-manifest "$lab_root/full-fidelity/${seed}.json" >/dev/null
    fi
    if test "$first_run" = false; then supabase db reset --workdir "$lab_root" --local --no-seed >/dev/null; fi
    first_run=false
    PGOPTIONS='-c client_min_messages=error' psql "$DB_URL" -X --set ON_ERROR_STOP=1 --file "$seed_sql" >"$lab_root/${mode_dir}-${seed}-seed.log" 2>&1
    if test -n "$full_sql"; then PGOPTIONS='-c client_min_messages=error' psql "$DB_URL" -X --set ON_ERROR_STOP=1 --file "$full_sql" >"$lab_root/${mode_dir}-${seed}-embeddings.log" 2>&1; fi
    node "$repo_root/decision-lab/src/d3.1-treatment-users-sql.mjs" "$world_root/world.json" > "$lab_root/treatment-users.sql"
    PGOPTIONS='-c client_min_messages=error' psql "$DB_URL" -X --set ON_ERROR_STOP=1 --file "$lab_root/treatment-users.sql" >"$lab_root/${mode_dir}-${seed}-treatments.log" 2>&1
    mkdir -p "$lab_root/output/$mode_dir/wave2.1"
    node "$repo_root/decision-lab/src/wave2.1-world-cli.mjs" --engine wave2.1 --config "$config" --output "$lab_root/output/$mode_dir/wave2.1/${seed}.json"
  done
done

supabase stop --project-id "$project_id" --no-backup >/dev/null 2>&1
started=false
if test "$modes" = "FAST_SIMULATION FULL_FIDELITY" && test "$seeds" = "backyrd-d1-basel-v1-2026 backyrd-d1-basel-v1-2026-2 backyrd-d1-basel-v1-2026-3"; then
  node "$repo_root/decision-lab/src/wave2.1-aggregate.mjs" --input "$lab_root/output" --full-fidelity "$lab_root/full-fidelity" --query-cache "$DECISION_LAB_EMBEDDING_CACHE_PATH" --output "$repo_root/decision-lab/baselines/wave2.1-retrieval-next-gen-v1.json"
else
  printf 'Wave 2.1 diagnostic subset completed; no promotable baseline was written.\n'
fi
printf 'Wave 2.1 comparison completed; disposable environment destroyed.\n'
