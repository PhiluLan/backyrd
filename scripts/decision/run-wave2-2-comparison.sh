...
lab_root="$(mktemp -d "${TMPDIR:-/tmp}/backyrd-wave2-2.XXXXXX")"
project_id="backyrd-wave2-2-retrieval"
started=false

cleanup() {
  if test "$started" = true; then supabase stop --project-id "$project_id" --no-backup >/dev/null 2>&1 || true; fi
  case "$lab_root" in
    "${TMPDIR:-/tmp}"/backyrd-wave2-2.*|/tmp/backyrd-wave2-2.*) rm -rf "$lab_root" ;;
...
sed -i.bak -e 's/port = 54321/port = 58921/' -e 's/port = 54322/port = 58922/' -e 's/shadow_port = 54320/shadow_port = 58920/' -e 's/port = 54329/port = 58929/' -e 's/port = 54323/port = 58923/' -e 's/port = 54324/port = 58924/' -e 's/port = 54327/port = 58927/' "$lab_root/supabase/config.toml"
...
for seed in backyrd-d1-basel-v1-2026 backyrd-d1-basel-v1-2026-2 backyrd-d1-basel-v1-2026-3; do
  ...
  node "$repo_root/decision-lab/src/wave2.2-world-cli.mjs" --config "$config" --output "$lab_root/output/$seed.json"
done

supabase stop --project-id "$project_id" --no-backup >/dev/null 2>&1
started=false
node "$repo_root/decision-lab/src/wave2.2-aggregate.mjs" --input "$lab_root/output" --full-fidelity "$lab_root/full-fidelity" --query-cache "$lab_root/full-fidelity/query-cache.json" --output "$repo_root/decision-lab/baselines/wave2.2-retrieval-breakthrough-v1.json"
printf 'Wave 2.2 FULL_FIDELITY comparison completed; disposable environment destroyed.\n'
