...
const pgJson=(value)=>Array.isArray(value)?`[${value.map(pgJson).join(", ")}]`:value&&typeof value==="object"?`{${Object.keys(value).sort().map((key)=>`${JSON.stringify(key)}: ${pgJson(value[key])}`).join(", ")}}`:JSON.stringify(value);
const digest=(value)=>createHash("sha256").update(pgJson(value)).digest("hex");
