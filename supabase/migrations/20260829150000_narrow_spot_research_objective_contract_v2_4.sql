-- Version the Research Agent after the bounded Basel v2.3 replay proved two
-- remaining boundaries: non-proposal facts could invalidate objective Pass A,
-- and provider dates were not reliable observation timestamps. Runtime v2.4
-- narrows Pass A and owns observation time. Prior evidence remains immutable.
...
begin
 ...
 v_scope:=jsonb_build_object('officialWebsite',v_url,'spotName',v_spot.name,'city',v_spot.city,'passes',jsonb_build_array('A','B'),'evidenceScopes',jsonb_build_array('SPOT','EVENT','PROGRAM','TEMPORARY','UNKNOWN_SCOPE'),'researchPolicyVersion','backyrd-spot-research-policy-v2.4');
...
begin
 ...
 v_scope:=jsonb_build_object('officialWebsite',v_spot.website,'spotName',v_spot.name,'city',v_spot.city,'passes',jsonb_build_array('A','B'),'evidenceScopes',jsonb_build_array('SPOT','EVENT','PROGRAM','TEMPORARY','UNKNOWN_SCOPE'),'researchPolicyVersion','backyrd-spot-research-policy-v2.4');
 v_hash:=encode(extensions.digest(convert_to(v_scope::text,'UTF8'),'sha256'),'hex');
...
comment on function public.backyrd_enqueue_spot_research_job_v1(uuid,text) is
  'Admin/Founder v2.1 enqueue with independently versioned v2.4 objective contract identity, active-job idempotency, and bounded schema recovery.';
comment on function public.backyrd_city_bootstrap_enqueue_research_v1(uuid) is
  'Service-only City Bootstrap adapter for the versioned v2.4 Research objective contract. It creates review proposals only and never Accepted Facts or N4.';
