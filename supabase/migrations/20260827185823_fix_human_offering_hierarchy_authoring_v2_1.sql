-- Keep the frozen Offering hierarchy strict while making a stale/manipulated
-- Human V2 client failure precise. The UI resolves conflicts explicitly.

create or replace function public.backyrd_offering_hierarchy_conflict_v1(p_value jsonb)
returns text language sql immutable set search_path=public,pg_catalog as $$
  select case
    when p_value->>'DRINKS'='NOT_AVAILABLE' and exists(
      select 1 from unnest(array['BEER','CRAFT_BEER','OWN_BREWED_BEER','WINE','COCKTAILS','COFFEE','NON_ALCOHOLIC']) as item(child)
      where p_value->>child='AVAILABLE'
    ) then 'DRINKS'
    when p_value->>'BEER'='NOT_AVAILABLE' and exists(
      select 1 from unnest(array['CRAFT_BEER','OWN_BREWED_BEER']) as item(child)
      where p_value->>child='AVAILABLE'
    ) then 'BEER'
    when p_value->>'FOOD'='NOT_AVAILABLE' and exists(
      select 1 from unnest(array['SNACKS','SMALL_PLATES','FULL_MEALS','BREAKFAST','BRUNCH','LUNCH','DINNER']) as item(child)
      where p_value->>child='AVAILABLE'
    ) then 'FOOD'
    else null
  end
...
revoke all on function public.backyrd_offering_hierarchy_conflict_v1(jsonb) from public,anon,authenticated;
grant execute on function public.backyrd_offering_hierarchy_conflict_v1(jsonb) to service_role;
...
begin
  ...
  for v_answer in select value from jsonb_array_elements(p_answers) loop
    ...
    if v_question.canonical_field_key='offering.availability' then
      ...
      if v_hierarchy_conflict is not null then
        raise exception 'offering_hierarchy_conflict:%',v_hierarchy_conflict using errcode='22023';
      end if;
    end if;
...
comment on function public.backyrd_human_spot_save_section_v2(uuid,text,jsonb,text,text,text,text,text,text) is
 'Founder/Admin Human V2 save with precise Offering hierarchy rejection. TEST/FIXTURE Spots remain limited to isolated Offering acceptance.';
