...
-- Keep list counters action-exact. Legacy email remains deliberately ambiguous
-- and is never auto-imported, but a consciously authored public email resolves
-- that workflow hint instead of leaving an unreachable open counter.
  ...
  from (
    select jsonb_build_object(
      ...
      'unconfirmedLegacyCount',(
        select count(*) from world_knowledge_private.legacy_import_values_v1 v
        where v.spot_id=s.id and v.disposition='PREFILL_REQUIRES_CONFIRMATION' and v.confirmed_claim_id is null and v.target_attribute_key is not null
          and not exists(select 1 from world_knowledge_private.claims c where c.spot_id=s.id and c.attribute_key=v.target_attribute_key)
      ),
      'ambiguousLegacyCount',(
        select count(*) from world_knowledge_private.legacy_import_values_v1 v
        where v.spot_id=s.id and v.disposition='REVIEW_REQUIRED'
          and not exists(
            select 1 from world_knowledge_private.claims c
            where c.spot_id=s.id
              and c.attribute_key=coalesce(v.target_attribute_key,case when v.source_field='email' then 'contact.public_email' end)
          )
      )
    ) row_value
    ...
    join public.spots s on s.id=f.spot_id
    left join lateral (
      select c.value from world_knowledge_private.claims c where c.spot_id=s.id and c.attribute_key='identity.name'
      order by c.last_changed_at desc,c.id desc limit 1
    ) name_claim on true
    left join lateral (
      select c.value from world_knowledge_private.claims c where c.spot_id=s.id and c.attribute_key='classification.primary_category'
      order by c.last_changed_at desc,c.id desc limit 1
