-- Scheduled/manual legacy workers may process only standalone Research jobs.
-- Intelligence Population jobs are owned exclusively by the run-scoped v2
-- claimant, so a LIVE_TICK can never drain an old or active Population run.
...
begin
 ...
 update public.backyrd_spot_research_passes_v2 p set state='QUEUED',updated_at=now()
 from public.backyrd_spot_research_jobs_v1 j
 where p.job_id=j.id and p.pass_key=j.current_pass and j.population_run_id is null
   and j.state='RUNNING' and j.lease_expires_at<=now();
 update public.backyrd_spot_research_jobs_v1
 set state='QUEUED',lease_token=null,lease_expires_at=null,runner_id=null,available_at=now(),updated_at=now()
 where population_run_id is null and state='RUNNING' and lease_expires_at<=now();
 select * into v_job from public.backyrd_spot_research_jobs_v1
 where population_run_id is null and state='QUEUED'
   and contract_version='backyrd-spot-research-agent-v2.1' and available_at<=now()
...
revoke all on function public.backyrd_claim_spot_research_job_v1(text,integer) from public,anon,authenticated,service_role;
grant execute on function public.backyrd_claim_spot_research_job_v1(text,integer) to service_role;

comment on function public.backyrd_claim_spot_research_job_v1(text,integer) is
  'Service-only standalone Research claim; Population jobs are exclusively leased by the run-scoped v2 contract.';
