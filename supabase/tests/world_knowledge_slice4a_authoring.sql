...
-- Registry@1.1 is the single runtime boundary used by UI and RPC writes.
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','hours.regular','KNOWN_VALUE','[{"day":"TUESDAY","intervals":[{"start":"17:00","end":"23:00"},{"start":"23:30","end":"02:00"}]}]'),'canonical multiple/overnight schedule rejected');
select pg_temp.assert(not world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','hours.regular','KNOWN_VALUE','[{"day":"TUE","intervals":[{"from":"17:00","to":"23:00"}]}]'),'former UI schedule shape accepted');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','hours.special','KNOWN_VALUE','[{"date":"2026-12-25","status":"CLOSED","intervals":[]},{"date":"2026-12-31","status":"OPEN","intervals":[{"start":"18:00","end":"02:00"}]}]'),'special hours rejected');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','rule.reservation','KNOWN_VALUE','{"mode":"CONDITIONAL","minimumPartySize":6,"days":["FRIDAY","SATURDAY"],"fromTime":"18:00","toTime":"23:00"}'),'reservation rule rejected');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','rule.external_food','KNOWN_VALUE','{"policy":"CONDITIONAL","exceptions":["Babynahrung"]}'),'consumption rule rejected');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','rule.pet_access','KNOWN_VALUE','{"indoor":"NOT_ALLOWED","outdoor":"ALLOWED","assistanceAnimals":"ALLOWED","notes":null}'),'pet rule rejected');
select pg_temp.assert(not world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','rule.pet_access','KNOWN_VALUE','{"indoor":"CONDITIONAL","outdoor":"ALLOWED","assistanceAnimals":"ALLOWED","notes":null}'),'unregistered pet state accepted');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','rule.age_access','KNOWN_VALUE','{"policy":"MINIMUM_AGE","minimumAge":18,"appliesFromTime":"21:00"}'),'age rule rejected');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','capacity.group_size_supported','KNOWN_VALUE','{"min":2,"max":40}'),'group range rejected');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','location.country_code','KNOWN_VALUE','"CH"') and not world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','location.country_code','KNOWN_VALUE','"Schweiz"'),'country boundary drift');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','location.timezone','KNOWN_VALUE','"Europe/Zurich"') and not world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','location.timezone','KNOWN_VALUE','"Zurich"'),'timezone boundary drift');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','contact.phone','KNOWN_VALUE','"+41786048842"') and not world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','contact.phone','KNOWN_VALUE','"078 604 88 42"'),'phone boundary drift');
select pg_temp.assert(world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','operation.takeaway','UNKNOWN',null) and world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','operation.takeaway','KNOWN_FALSE','false') and not world_knowledge_private.attribute_value_valid_v1('backyrd.world-knowledge.registry@1.1','operation.takeaway','UNKNOWN','false'),'unknown and false collapsed');
...
select pg_temp.assert((public.world_authoring_set_applicability_v1((select (payload->>'spotId')::uuid from wk4a_created limit 1),'operation.takeaway','NOT_APPLICABLE','basic-na')->>'applicability')='NOT_APPLICABLE','not-applicable authoring event failed');
...
select pg_temp.assert((public.world_authoring_get_spot_v1((select (payload->>'spotId')::uuid from wk4a_created limit 1))#>>'{applicability,operation.takeaway}')='NOT_APPLICABLE','applicability not restored');
