...
select pg_temp.assert((select source_scope->>'researchPolicyVersion'='backyrd-spot-research-policy-v2.4' from public.backyrd_spot_research_jobs_v1 where spot_id=(select id from public.spots where google_place_id='test-google-place-1' and status='approved')),'City Research policy version missing from durable source scope');
