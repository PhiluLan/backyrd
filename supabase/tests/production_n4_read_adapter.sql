...
select public.backyrd_process_user_intelligence_work_v1(10);
select pg_temp.assert((select count(*)=0 from public.backyrd_user_intelligence_nodes_v2 where user_id=pg_temp.uuid('n4-adapter-user') and concept_key is null),'no missing-N4 concept is imputed');
select pg_temp.assert((select count(*)>0 from public.backyrd_user_intelligence_nodes_v2 where user_id=pg_temp.uuid('n4-adapter-user') and concept_key in ('character.authentic_character','vibe.lively')),'comparative runtime uses canonical N4 concepts');
select pg_temp.assert((select count(*)=0 from public.backyrd_user_intelligence_nodes_v2 where user_id=pg_temp.uuid('n4-adapter-user') and concept_key='vibe.lively' and polarity='NEGATIVE'),'comparative negative is not promoted without independent concept-absent comparison evidence');
reset role;
