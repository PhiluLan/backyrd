...
select pg_temp.assert((public.world_owner_submit_claim_v1(pg_temp.id('wk-basic-spot'),'identity.name','KNOWN_VALUE','"Basic Casa"',clock_timestamp(),null,null,'PUBLIC',null,'basic-name')->>'verificationMethod')='OWNER_CONFIRMED','Basic owner write failed');
select pg_temp.assert((public.world_confirm_claim_v1(((public.world_owner_submit_claim_v1(pg_temp.id('wk-basic-spot'),'identity.name','KNOWN_VALUE','"Basic Casa"',clock_timestamp(),null,null,'PUBLIC',null,'basic-name')->>'claimId')::uuid),'basic-name-confirmation')->>'claimUnchanged')::boolean,'confirmation did not preserve claim semantics');
