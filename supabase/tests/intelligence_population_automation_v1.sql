...
select pg_temp.population_assert(not has_function_privilege('anon','public.backyrd_activate_intelligence_population_concurrency_four_v1()','execute'),'anon can tune Population concurrency');
select pg_temp.population_assert(not has_function_privilege('authenticated','public.backyrd_activate_intelligence_population_concurrency_four_v1()','execute'),'authenticated/admin browser can tune Population concurrency');
select pg_temp.population_assert(has_function_privilege('service_role','public.backyrd_activate_intelligence_population_concurrency_four_v1()','execute'),'service worker cannot tune Population concurrency');
...
select pg_temp.population_assert(pg_get_function_arguments('public.backyrd_activate_intelligence_population_concurrency_four_v1()'::regprocedure)='','concurrency tuner accepts caller-controlled arguments');
...
do $$begin
  ...
  begin perform public.backyrd_activate_intelligence_population_concurrency_four_v1();raise exception 'anon concurrency tuning unexpectedly succeeded';exception when insufficient_privilege then null;end;
end$$;
reset role;
...
do $$begin
  ...
  begin perform public.backyrd_activate_intelligence_population_concurrency_four_v1();raise exception 'authenticated concurrency tuning unexpectedly succeeded';exception when insufficient_privilege then null;end;
end$$;
reset role;
...
values('d6b5bfb0-4236-44f9-aa80-68be421bc289','basel-intelligence-population-automation-test','basel','Basel','{}','{}','{"phase":"FULL_LAUNCH_CURATION","researchConcurrencyLimit":2,"researchQueueBatchSize":5,"researchCoverageTarget":415,"discoveryEnabled":false}','backyrd-intelligence-population-v1',repeat('e',40),'INTELLIGENCE','RUNNING','62000000-0000-4000-8000-000000000001',now());

set local role service_role;
select set_config('request.jwt.claims','{"role":"service_role"}',true),set_config('request.jwt.claim.role','service_role',true);
select pg_temp.population_assert((public.backyrd_activate_intelligence_population_concurrency_four_v1()->>'researchConcurrencyLimit')::integer=4,'authorized service worker could not set measured concurrency four');
select pg_temp.population_assert((public.backyrd_intelligence_population_tick_control_v1('d6b5bfb0-4236-44f9-aa80-68be421bc289','CLAIM','62000000-0000-4000-8000-000000000003')->>'claimed')::boolean,'authorized service worker could not claim tick lease');
select pg_temp.population_assert(not (public.backyrd_intelligence_population_tick_control_v1('d6b5bfb0-4236-44f9-aa80-68be421bc289','CLAIM','62000000-0000-4000-8000-000000000004')->>'claimed')::boolean,'overlapping service tick acquired a second lease');
do $$begin
  begin perform public.backyrd_intelligence_population_tick_control_v1('d6b5bfb0-4236-44f9-aa80-68be421bc289','RELEASE','62000000-0000-4000-8000-000000000004');raise exception 'foreign lease token released Population tick';exception when serialization_failure then null;end;
end$$;
select pg_temp.population_assert((public.backyrd_intelligence_population_tick_control_v1('d6b5bfb0-4236-44f9-aa80-68be421bc289','RELEASE','62000000-0000-4000-8000-000000000003')->>'released')::boolean,'authorized service worker could not release its lease');
reset role;
