...
select pg_temp.assert(position('backyrd-spot-research-policy-v2.7' in pg_get_functiondef('public.backyrd_city_bootstrap_enqueue_research_v1(uuid)'::regprocedure))>0,'City enqueue policy was not versioned');
select pg_temp.assert(has_function_privilege('service_role','public.backyrd_finalize_spot_research_pass_v3(uuid,uuid,text,jsonb,jsonb,jsonb)','EXECUTE'),'service role cannot execute v2.7 finalizer');
