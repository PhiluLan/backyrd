...
export function planProviderCreditCanary(activeJobs:Array<{spot_id?:unknown;state?:unknown}>,pendingSpotIds:unknown[],limit=5){
  const failures:string[]=[],queuedCounts=new Map<string,number>();
  for(const job of activeJobs){const state=clean(job.state),spotId=clean(job.spot_id);if(state!=="QUEUED")failures.push("ACTIVE_JOB_NOT_QUEUED");if(!spotId)failures.push("QUEUED_JOB_SPOT_INVALID");else queuedCounts.set(spotId,(queuedCounts.get(spotId)??0)+1);}
  if(activeJobs.length>limit*2)failures.push("QUEUED_JOB_BOUND_EXCEEDED");
  if([...queuedCounts.values()].some((count)=>count>2))failures.push("QUEUED_JOB_DUPLICATE_COHORT");
  const queuedSpotIds=[...queuedCounts.keys()].sort();if(queuedSpotIds.length>limit)failures.push("QUEUED_SPOT_BOUND_EXCEEDED");
  const canarySpotIds=[...queuedSpotIds];for(const value of pendingSpotIds){const spotId=clean(value);if(spotId&&!canarySpotIds.includes(spotId)&&canarySpotIds.length<limit)canarySpotIds.push(spotId);}
  if(canarySpotIds.length!==limit)failures.push("CANARY_SIZE_INVALID");
...
if (import.meta.main) Deno.serve(async (request) => {
  ...
  try {
    ...
    if(action==="RESUME_PROVIDER_CREDIT_CANARY") {
      ...
      const run=matching[0],[{data:active,error:activeError},{data:pending,error:pendingError}]=await Promise.all([
        db.from("backyrd_spot_research_jobs_v1").select("id,spot_id,state").eq("population_run_id",run.id).in("state",["QUEUED","RUNNING"]),
        db.from("backyrd_spot_intelligence_population_v1").select("spot_id").eq("run_id",run.id).eq("terminal_state","PENDING").order("spot_id").limit(5)
      ]);if(activeError||pendingError)throw activeError??pendingError;const plan=planProviderCreditCanary(active??[],(pending??[]).map((item:any)=>item.spot_id));if(!plan.ok)return json({ok:false,error:"provider_credit_canary_precondition_failed",failures:plan.failures,queuedJobs:(active??[]).length,pendingCanary:pending?.length??0},409);
      const acknowledgedBefore=new Date().toISOString(),canarySpotIds=plan.canarySpotIds,targetConfiguration={...run.target_configuration,providerFailureCircuitReset:{failureCode:"research_provider_failed",acknowledgedBefore,remediation:"CANONICAL_PROVIDER_CREDIT_RESTORED_V1",canarySize:5,canarySpotIds,checkpointQueuedSpotIds:plan.queuedSpotIds}};
    ...
    if(action==="RESUME_RESEARCH_COVERAGE_CANARY") {
      const healthResponse=await fetch(`${url}/functions/v1/research-spot-worker`,{method:"POST",headers:{authorization:`Bearer ${serviceKey}`,"content-type":"application/json"},body:JSON.stringify({action:"PROVIDER_HEALTH"})}),health=await healthResponse.json().catch(()=>({ok:false,errorCode:"provider_health_malformed"}));
      if(!healthResponse.ok||health?.ok!==true)return json({ok:false,error:"canonical_research_provider_unhealthy",providerErrorCode:clean(health?.errorCode)||"unknown"},503);
      const {data:runs,error:runError}=await db.from("backyrd_city_bootstrap_runs_v1").select("id,status,stop_reason,target_configuration").eq("mode","INTELLIGENCE").eq("status","PAUSED").eq("target_configuration->>phase","FULL_LAUNCH_CURATION").limit(2);if(runError)throw runError;
      const matching=(runs??[]).filter((run:any)=>run.stop_reason==="CIRCUIT_BREAKER:SYSTEMATIC_RESEARCH_FAILURE:research_fact_coverage_incomplete");if(matching.length!==1)return json({ok:false,error:"research_coverage_paused_run_invalid",matchingRuns:matching.length},409);
      const run=matching[0],[{data:active,error:activeError},{data:pending,error:pendingError}]=await Promise.all([
        db.from("backyrd_spot_research_jobs_v1").select("id,spot_id,state,provider_response_id").eq("population_run_id",run.id).in("state",["QUEUED","RUNNING"]),
        db.from("backyrd_spot_intelligence_population_v1").select("spot_id").eq("run_id",run.id).eq("terminal_state","PENDING").order("spot_id").limit(5)
      ]);if(activeError||pendingError)throw activeError??pendingError;
      if((active??[]).some((job:any)=>job.state!=="QUEUED"||clean(job.provider_response_id)))return json({ok:false,error:"research_coverage_checkpoint_not_clean",activeJobs:(active??[]).length},409);
      const plan=planProviderCreditCanary(active??[],(pending??[]).map((item:any)=>item.spot_id));if(!plan.ok)return json({ok:false,error:"research_coverage_canary_precondition_failed",failures:plan.failures,queuedJobs:(active??[]).length,pendingCanary:pending?.length??0},409);
      const acknowledgedBefore=new Date().toISOString(),targetConfiguration={...run.target_configuration,researchFactCoverageCircuitReset:{failureCode:"research_fact_coverage_incomplete",acknowledgedBefore,remediation:"KEYED_REQUIRED_FACT_EVIDENCE_SCHEMA_V1",canarySize:5,canarySpotIds:plan.canarySpotIds,checkpointQueuedSpotIds:plan.queuedSpotIds}};
      const resumed=await db.from("backyrd_city_bootstrap_runs_v1").update({status:"RUNNING",stop_reason:null,target_configuration:targetConfiguration}).eq("id",run.id).eq("status","PAUSED").eq("stop_reason","CIRCUIT_BREAKER:SYSTEMATIC_RESEARCH_FAILURE:research_fact_coverage_incomplete").select("id,status").single();if(resumed.error)throw resumed.error;
      return json({ok:true,runId:run.id,status:"RUNNING",schedulerActivated:false,canarySize:5,providerHealth:"PASS",checkpointQueuedSpots:plan.queuedSpotIds.length,wroteCanonicalFacts:false});
    }
    ...
    if(action==="POPULATION_TICK") {
      ...
      try {
        ...
        const resetByCode={"research_source_invalid:0":run.target_configuration?.sourceUrlFailureCircuitReset?.acknowledgedBefore,"research_provider_failed":run.target_configuration?.providerFailureCircuitReset?.acknowledgedBefore,"research_fact_coverage_incomplete":run.target_configuration?.researchFactCoverageCircuitReset?.acknowledgedBefore},systemicFailure=systemicResearchFailure(unresolvedResearchFailures(failedJobs??[],resetByCode));
