...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if(action==="STAGE_PILOT") {
      ...
      if(prior){if(prior.canonical_repository_commit!==commit||prior.requested_by!==requestedBy)return json({ok:false,error:"pilot_replay_contract_mismatch"},409);const {data:priorCandidates}=await db.from("backyrd_city_bootstrap_candidates_v1").select("lifecycle_state").eq("run_id",prior.id);const eligible=(priorCandidates??[]).filter((candidate:any)=>["PRODUCT_ELIGIBLE","PUBLISHED"].includes(candidate.lifecycle_state)).length;return json({ok:prior.status==="RUNNING",runId:prior.id,status:prior.status,verdict:prior.status==="RUNNING"?"PASS":"FAIL",replayed:true,counts:{requested:candidates.length,providerCalls:0,eligible}},prior.status==="RUNNING"?200:409);}
