...
const MODEL_NAME = "backyrd_decision_wave1_candidate";
const MODEL_VERSION = "1.0.0-candidate";
const WAVE1_ENGINE_CANDIDATE = "decision-wave1-intent-constraints-v1";
...
export function applyUserHardConstraintEligibilityV1(
  ...
  for (const candidate of candidates) {
    ...
    let exclusion: HardConstraintExclusionV1 | null = null;
    if (intent.hardConstraints.requiredPlaceTypes.length > 0 && !intent.hardConstraints.requiredPlaceTypes.includes(placeType)) {
      exclusion = { spot_id: candidate.spot_id, constraint: "HARD_CATEGORY", expected: intent.hardConstraints.requiredPlaceTypes, observed: placeType, evidenceStatus: "KNOWN" };
    } else if (intent.hardConstraints.excludedPlaceTypes.includes(placeType)) {
      exclusion = { spot_id: candidate.spot_id, constraint: "CATEGORY_EXCLUSION", expected: intent.hardConstraints.excludedPlaceTypes, observed: placeType, evidenceStatus: "KNOWN" };
    } else if (intent.hardConstraints.openNow && candidate.is_open_now !== true) {
      exclusion = { spot_id: candidate.spot_id, constraint: "OPEN_NOW", expected: true, observed: candidate.is_open_now, evidenceStatus: candidate.is_open_now === null ? "UNKNOWN" : "KNOWN" };
    }
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    // Product and Distribution eligibility have already run. User constraint
    // eligibility is applied inside fusion after candidate materialization but
    // before any score calculation, sort or diversity can compensate.
