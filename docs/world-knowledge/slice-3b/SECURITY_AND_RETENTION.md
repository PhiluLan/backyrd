...
Current projection updates are serialized separately per Spot and compare accepted versions, `as_of`, and ledger cutoff. An older historical rebuild cannot replace a newer pointer; same-time re-resolution can advance only for a later input cutoff. Request replay is locked by idempotency identity and returns the original validated result before inspecting newer ledger state. Canonical manifest creation is separately locked by Spot plus the complete semantic input identity, so different request keys safely converge without globally serializing unrelated Spots. The SQL suite and real two-process harness prove same-key replay, cross-key convergence, different-input separation and both pointer orders without claiming that the shadow path is Production-enabled.

The public shadow entry point remains `SECURITY DEFINER` because `service_role` is intentionally denied direct execution of the private helpers and private ledger mutation; the function is the narrow audited capability that performs the atomic multi-table transaction. It has `search_path=''`, schema-qualifies every relation, checks the synthetic/allowlisted Spot boundary for new work, and grants execute only to `service_role`. Exact successful retries may outlive the allowlist window because they perform no new resolution or pointer effect and fully validate the already-authorized stored manifest. `PUBLIC`, `anon` and `authenticated` execute remain revoked, and catalog-driven tests reject any indirect helper path.
