...
Status: implementation and disposable-local validation complete; Production mutation, canary and Founder physical acceptance not performed.
...
Known examples dry-run as: `gemütlich → mood.cozy`, `cozy → mood.cozy`, `lebhaft → mood.lively`, `a/b/test → INVALID`, `cocktails → INVALID`. Unknown expressions remain `UNRESOLVED`. Exact final Production bucket counts must be captured immediately before mutation because the Phase 1 anonymous read could not safely establish contributor/moderation dimensions and Production may change.
...
| Full `supabase db reset` | BLOCKED before Mood migration by pre-existing `20260826120000_production_decision_fixture_cleanup_v1.sql` fixture identity/provenance mismatch |
...
The existing clean-bootstrap blocker must be fixed in its owning workstream before release; it was not bypassed or rewritten here.
