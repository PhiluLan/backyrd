...
- Backyrd deterministically validates typed values, compares accepted facts (`NEW/SAME/CONFLICT/STALE/UNSUPPORTED`), applies the existing official-source confidence policy and builds proposals. The existing evidence scope remains `SPOT`, `EVENT`, `PROGRAM`, `TEMPORARY`, or `UNKNOWN_SCOPE`. Research additionally attributes the evidence subject as `SPOT`, `SUBVENUE`, `EVENT`, `PROGRAM`, `TEMPORARY`, `SERVICE`, `OFFERING`, `TENANT`, `PERSON`, `OTHER`, or `AMBIGUOUS`, plus `PERSISTENT`, `TEMPORARY`, or `UNKNOWN` durability. These are research guards, not new Product, N4, Offering, or Purpose semantics.
- Policy `backyrd-spot-research-policy-v2.5` permits a proposal only when temporal scope, named subject, durability and the field-specific evidence validator all resolve deterministically to the canonical persistent Spot. Model-declared scope or confidence cannot override that result. `identity.name`, `contact.website`, `category.primary` (plus its deterministic `place_type` adapter), `activity.types`, and `accessibility.capabilities` remain the unchanged objective allowlist. Opening hours and qualitative/contextual Pass-B fields remain extraction-only. The policy identity is part of the durable source-scope hash, so a replay is explicit and independently idempotent.
- Activity enum members require direct lexical support; services or courses are not semantically promoted to `WORKSHOP`. Accessibility requires an explicit persistent venue capability; event-limited, conditional or temporary assistance remains trace-only. Subvenues, tenants/operators, staff and ambiguous subjects never inherit to their parent Spot. Contact websites must be canonical public HTTPS URLs on the exact official host (root and `www` are equivalent).
- Its strict provider schema couples `SUPPORTED` to a non-null field-specific typed value and couples `UNKNOWN` or `UNSUPPORTED` to `null`. This prevents a structurally valid provider response from violating the deterministic support/value invariant discovered during the bounded Basel v2.2 replay.
- Provider-supplied `observed_at` is fixed to `null`; Backyrd owns the auditable system observation timestamp. The bounded v2.3 replay proved that provider-inferred publication or event dates are not a reliable observation-time contract.
...
The existing service-only `backyrd_finalize_spot_research_pass_v3` endpoint now revalidates the Entity/Subentity resolution and exact public official host in Postgres, then persists one complete pass, scoped extractions, sources, deterministic proposals, attempt metadata and next disposition in one transaction. Its pre-v2.5 implementation is retained only as a non-executable internal transaction primitive. Every response declares `canonicalWrite=false`. Existing proposal review, accepted facts, N4 and Gold Readiness are unchanged.
