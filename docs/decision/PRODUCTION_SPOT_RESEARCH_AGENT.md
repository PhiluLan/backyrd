...
- Policy `backyrd-spot-research-policy-v2.4` limits review proposals and Pass A to objective `identity.name`, `contact.website`, `category.primary` (plus its deterministic `place_type` adapter), `activity.types`, and `accessibility.capabilities`. Opening hours are excluded from routine provider research after a Basel subentity failure; their historical evidence remains auditable. Qualitative or contextual Pass-B evidence remains extraction-only. The policy identity is part of the durable source-scope hash, so a policy replay is explicit and independently idempotent.
