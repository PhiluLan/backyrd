...
- Policy `backyrd-spot-research-policy-v2.3` limits review proposals to objective `identity.name`, `contact.website`, `category.primary` (plus its deterministic `place_type` adapter), `opening.regular`, `activity.types`, and `accessibility.capabilities`. Qualitative or contextual Pass-B evidence remains an auditable extraction but cannot create routine proposals. The policy identity is part of the durable source-scope hash, so a policy replay is explicit and independently idempotent.
