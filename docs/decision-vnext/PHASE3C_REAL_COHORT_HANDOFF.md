...
The imported Registry-2.1 handoff's authorized `purpose.primary_visit` fact and immutable snapshot classification feed the versioned, evaluation-only core-intent mapping. `EAT_DRINK` proves only the broad domain; a specific intent additionally requires a closed primary-category or place-type signal. Candidate logic contains no spot names or spot IDs. `offering.onsite` is reported with its `PART_OF_SPOT` or `EMBEDDED_FACILITY` relationship but cannot create core-intent confirmation. A candidate can enter `ELIGIBLE_CONFIRMED` only when core intent, target location and every hard condition are confirmed. Missing specific evidence remains an unconfirmed fallback rather than an automatic incompatibility, missing mapping authority remains `NOT_CONFIGURED`, a confirmed different primary-purpose domain is incompatible, and disputed evidence remains visible without an automatic winner.
