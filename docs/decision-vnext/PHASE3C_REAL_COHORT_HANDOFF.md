...
The imported Registry-2.1 handoff's authorized `purpose.primary_visit` fact and immutable snapshot classification feed the versioned, evaluation-only core-intent mapping. `EAT_DRINK` proves only the broad domain; a specific intent additionally requires a closed primary-category or place-type signal. The resolution order is explicit: an authorized matching specific classification confirms the intent; an authorized classification listed as incompatible for that intent rejects it; only genuinely missing, untrusted or outside-policy specific evidence remains `UNKNOWN`. The Phase-3C coffee mapping therefore distinguishes a primary café from known restaurant/pub, wine-bar, sport and nature classifications without any name-, ID- or position-specific exception. `offering.onsite` is reported with its `PART_OF_SPOT` or `EMBEDDED_FACILITY` relationship but cannot create core-intent confirmation. A candidate can enter `ELIGIBLE_CONFIRMED` only when core intent, target location and every hard condition are confirmed. Missing mapping authority remains `NOT_CONFIGURED`, and disputed evidence remains visible without an automatic winner.
...
For the bound five-spot Founder evaluation fixture, both sides of the controlled drinks flip resolve to two specifically confirmed candidates (Volta Bräu and Consum Weinbar), one broad-domain fallback without drinks-specific confirmation (Café Frühling), and two incompatible primary-purpose domains (ELYS Boulderloft and Tierpark Lange Erlen). The different quiet/date and lively/friends evidence is shown separately while core intent, Basel and hard constraints remain fixed. This result follows the World handoff and closed policy matrix rather than a name-, ID- or position-specific exception. The regular Founder view translates limitations and reasons into German; raw reason codes and hashes remain available only after deliberately opening the expert view.
