...
`SituationalContextSnapshot` has three sections: explicit one-decision input, server-bound authority and derived values. The resolver requires an explicit `authorizedLocationScope`; an authenticated actor alone is never location authority. Client location, authorized scope and their `MATCH` proof are independently hashed into Context. A mismatch or missing scope fails closed. Budget, available time, weather and exploration stay in `explicit` as strictly validated placeholder dimensions (including distinct `UNKNOWN` and `NOT_CONFIGURED` states); they are not relabeled as derived or silently discarded. A `DERIVED` value requires at least one source hash. The snapshot declares `rawLocationPersisted: false` and `writesUserIntelligence: false`; only an independently authorized User observation may later create durable learning.
...
The only rules are distribution allowed, explicit city and explicit open-now. Each check binds rule/ruleset, evidence, outcome, reason, local unknown policy and proof hash. The versioned opening evaluator uses the spot's IANA timezone, DST-aware local date/time, regular and date-specific special hours, and overnight intervals. Special hours win for their local date. Venue hours and kitchen hours remain distinct. Only `SPOT`/`VENUE` current states may override the whole venue; an area state such as terrace closure cannot. Blocking temporal conflicts become `disputed`, absent authorization becomes `not_authorized`, actual absence becomes `unknown`, and an otherwise unresolvable expired current state remains `expired`. Phase 1 uses an explicitly unapproved synthetic source policy; no Production source policy is implied. For `openNow=true`, anything other than authoritatively open fails; unknown-family states are recorded as unknown and handled by the rule's fixture `fail` policy.
...
The v3 manifest binds Decision contracts, World port/registry/rules, User projection/manifest, Context, generator/pool, eligibility/unknown policy, opening evaluator/source policy, ranking adapter, fixture weight, confidence, evidence, explanation and exploration state. Every supported Phase-1 version is allowlisted; a syntactically valid future key fails closed. Request, envelope, snapshots, candidates/evidence, pool, checks, eligibility results, fits, confidence, recommendations and result are content-addressed.

`validateDecisionResultIntegrity` is recursive rather than an outer-hash check. It validates unique identities, every nested content hash, World/User/Context/Pool/Manifest bindings, recomputes central Eligibility, replays the selected baseline fit/order, reconstructs Confidence, re-authorizes and deterministically rerenders Explanation, then validates the Result hash. Negative tests mutate inner semantics and recompute all affected outer hashes; replay still rejects them.
