...
type Status={active?:Array<{id:string;type:string;public_explanation:string;ends_at:string|null}>;
...
export default function SafetyNotifications(){
 ...
 return <View style={st.root}><Stack.Screen options={{headerShown:false}}/>
  ...
  {loading?<View style={st.center}><ActivityIndicator color="#FF4F8B"/></View>:
  <ScrollView contentContainerStyle={[st.content,{paddingBottom:insets.bottom+40}]}
   ...
   {notices.map(n=><Pressable key={n.notice_id} onPress={()=>void open(n)}
    ...
    <View style={st.icon}><Ionicons name={n.notice_type.includes("appeal")?"checkmark-circle-outline":"shield-checkmark-outline"} size={22} color="#FF7DA7"/></View>
...
const st=StyleSheet.create({
 root:{flex:1,backgroundColor:"#09090B"},header:{paddingHorizontal:18,paddingBottom:16,flexDirection:"row",gap:12,alignItems:"center"},
 back:{width:42,height:42,borderRadius:21,alignItems:"center",justifyContent:"center",backgroundColor:"rgba(255,255,255,.06)"},
 eyebrow:{color:"#FF7DA7",fontSize:11,fontWeight:"900",letterSpacing:1.2},title:{color:"#fff",fontSize:28,fontWeight:"900"},
 ...
 emptyTitle:{color:"#fff",fontSize:18,fontWeight:"850"},muted:{color:"rgba(255,255,255,.62)",lineHeight:21},
 ...
 cardTitle:{color:"#fff",fontSize:16,fontWeight:"850",marginBottom:6},date:{color:"rgba(255,255,255,.38)",fontSize:12,marginTop:8},
 action:{color:"#FF7DA7",fontSize:13,fontWeight:"800",marginTop:9}
});
