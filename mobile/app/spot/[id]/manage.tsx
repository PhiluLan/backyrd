...
export default function SpotManageScreen() {
  ...
  async function saveAll() {
    ...
    try {
      ...
      const { data: saveData, error: saveErr } = await supabase.rpc("update_owner_spot_mobile_moderated_v1", {
      ...
      const isSafetyRestriction =
        rawMessage.includes("SAFETY_WRITE_RESTRICTED");

      if (isSafetyRestriction) {
        console.info(
          "Owner editing blocked by Safety enforcement.",
        );

        Alert.alert(
          "Bearbeitung derzeit nicht möglich",
          "Dein Konto wurde vorübergehend eingeschränkt. Deshalb kannst du die Angaben dieses Spots aktuell nicht ändern. Im Safety Center siehst du den Grund und wie lange die Einschränkung gilt.",
          [
            {
              text: "Verstanden",
              onPress: () => router.back(),
        ...
        console.error("Owner spot save failed:", e);

        Alert.alert(
          "Änderungen nicht gespeichert",
          "Beim Speichern ist etwas schiefgelaufen. Bitte versuche es später erneut.",
        );
      }
    } finally {
      setSaving(false);
