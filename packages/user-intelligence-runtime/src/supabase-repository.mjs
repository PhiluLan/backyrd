...
export class SupabaseUserIntelligenceRepository {
  ...
  async persistAtomically({ userId, reason, sourceWatermark, input, card, nodes, ledger, dispositions, runtimeVersion, workIds, leaseToken }) {
    ...
    const { data, error } = await this.client.rpc("backyrd_persist_shared_user_intelligence_v3", { p_user_id:userId,p_runtime_version:runtimeVersion,p_input_contract_version:"backyrd-production-input-adapter-v3+temporal-attribution-v1",p_source_watermark:sourceWatermark,p_source_hash:hash(input),p_snapshot_hash:card.userCardHash,p_card:card,p_nodes:nodes,p_ledger:ledger,p_dispositions:persistedDispositions,p_work_ids:workIds,p_lease_token:leaseToken }); fail(error); return { snapshotId:data, snapshotHash:card.userCardHash, reason };
  }
