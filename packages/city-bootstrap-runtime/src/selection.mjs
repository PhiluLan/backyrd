...
const stableKey = (candidate) => candidate.identityKey ?? `${candidate.sourceFamily ?? "UNKNOWN"}:${candidate.sourceIdentity ?? "UNKNOWN"}:${candidate.name ?? ""}:${candidate.lat ?? ""}:${candidate.lng ?? ""}`;
...
export function selectLaunchCohort(candidates, existingSpots, config, targetNewCount) {
  ...
  while (selected.length < targetNewCount && remaining.length) {
    remaining.sort((a, b) => {
      ...
      return categoryNeedB - categoryNeedA || geoA - geoB || (b.sourceQuality ?? 0) - (a.sourceQuality ?? 0) || stableKey(a).localeCompare(stableKey(b));
    });
...
export function selectRepresentativePilot(candidates, size = 30) {
  ...
  for (const rows of pools.values()) rows.sort((a, b) => stableKey(a).localeCompare(stableKey(b)));
