export function createSpotResearchRepository(service) {
  ...
  return Object.freeze({
    ...
    fail: ({ jobId, leaseToken, passKey }, retryable, failureCode) => rpc("backyrd_fail_spot_research_pass_v2", { p_job_id: jobId, p_lease_token: leaseToken, p_pass_key: passKey, p_retryable: retryable, p_failure_code: failureCode }),
    finalizePass: ({ jobId, leaseToken, passKey }, extractions, proposals, metadata) => rpc("backyrd_finalize_spot_research_pass_v3", { p_job_id: jobId, p_lease_token: leaseToken, p_pass_key: passKey, p_extractions: extractions, p_proposals: proposals, p_provider_metadata: metadata }),
    async loadContext(claim) {
      const [spotResult, catalogResult, factsResult] = await Promise.all([
        ...
        service.from("backyrd_spot_fact_catalog_v1").select("field_key,value_kind,allowed_values,engine_role,contract_version"),
