import { buildDeterministicProposalPlan, createBackgroundResearchResponse, retrieveBackgroundResearchResponse, validateResearchEvidence } from "./index.mjs";
...
export async function processOneResearchJob({ repository, apiKey, runnerId, provider = {}, pollDelaySeconds = 4 }) {
  ...
  const identity = { jobId: claim.jobId, leaseToken: claim.leaseToken, passKey: claim.passKey };
  try {
    const context = await repository.loadContext(claim);
    let response;
    if (claim.providerResponseId) response = await (provider.retrieve ?? retrieveBackgroundResearchResponse)(claim.providerResponseId, { apiKey });
    else {
      const attempt = await repository.beginAttempt(identity);
      response = await (provider.create ?? createBackgroundResearchResponse)(context, { apiKey, model: claim.model, passKey: claim.passKey, idempotencyKey: attempt.attemptToken });
    ...
    if (pendingStatuses.has(response.providerStatus)) {
      ...
      return { state: "RUNNING", phase: `PASS_${claim.passKey}_RUNNING`, jobId: claim.jobId, providerStatus: response.providerStatus };
    }
    ...
    const result = await repository.finalizePass(identity, plan.extractions, plan.proposals, {
      providerResponseId: response.providerResponseId, providerStatus: response.providerStatus,
      inputTokens: response.usage.inputTokens, outputTokens: response.usage.outputTokens, totalTokens: response.usage.totalTokens,
      webSearchCalls: response.webSearchCalls, latencyMs: response.transportLatencyMs, inputBytes: response.inputBytes ?? 0
    });
    return { ...result, jobId: claim.jobId, passKey: claim.passKey };
  } catch (error) {
    const code = safeCode(error);
    const failed = await repository.fail(identity, isRetryableResearchFailure(code), code);
    return { state: failed?.state ?? "FAILED", phase: failed?.phase, jobId: claim.jobId, passKey: claim.passKey, failureCode: code, retry: Boolean(failed?.retry) };
  }
}
