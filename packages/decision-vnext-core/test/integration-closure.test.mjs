...
import { NEUTRAL_SUBJECT_BINDING_HASH, parseRelevantUserProjection } from "@backyrd/user-intelligence-vnext-core";
...
test("missing World and User degradation modes are explicit and subject-bound", async () => {
  ...
  const neutralForAnotherSubject = await readRelevantUserProjection(fixed, { ...baseRequest, actor: { ...baseRequest.actor, subjectBindingHash: "1".repeat(64) } });
  assert.equal(neutralForAnotherSubject.subjectBindingHash, NEUTRAL_SUBJECT_BINDING_HASH, "privacy-neutral projections must not preserve a linkable subject binding");
});
...
test("context flips remain bound while neutral projections remain unlinkable across subjects", () => {
  ...
  assert.equal(first.userBinding.subjectBindingHash, NEUTRAL_SUBJECT_BINDING_HASH);
  assert.equal(userBound.userBinding.subjectBindingHash, NEUTRAL_SUBJECT_BINDING_HASH);
  assert.notEqual(first.envelopeHash, userBound.envelopeHash, "server actor authority remains bound outside the privacy-neutral projection");
  assert.equal(synthetic.worldHash, before);
});
