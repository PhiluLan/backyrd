...
import {
  ...
  canonicalJson, contentHash, createCanonicalFounderLiveEvaluator, createFounderLiveHttpHandler, createFounderLivePostDeployEvidence, executeFounderLiveDecision, executeFounderLiveDualRun, generateSyntheticWorld,
} from "../dist/index.js";
import { SYNTHETIC_WORLD_SOURCE_POLICY } from "../dist/synthetic-world-policy.js";

const ACTOR = Object.freeze({ userId: "founder-local-user", subjectBindingHash: contentHash("founder-live-subject"), authenticationContextHash: contentHash("founder-live-auth"), expertAccess: true });
...
function ports(overrides = {}) {
  ...
  return {
    state,
    auth: { contractVersion: "backyrd.decision-vnext.founder-live-auth-port@1.0", async authenticate(token) { return token === "local-founder-token" ? ACTOR : null; } }, allowlist: { contractVersion: "backyrd.decision-vnext.founder-live-allowlist-port@1.0", async authorize() { return true; } },
...
test("server-bound execution is deterministic, port-only, evaluation-only and idempotent", async () => {
  ...
  const one = await executeFounderLiveDecision(value, ACTOR, p); const two = await executeFounderLiveDecision(value, ACTOR, p);
...
test("A-D, family, bouldering, flip, alternative, reject and replay use the same API", async () => {
  ...
  for (const [id, text, intent, city] of scenarios) { const result = await executeFounderLiveDecision(request(id, text), ACTOR, ports()); assert.equal(result.response.understood.primaryIntent, intent); assert.equal(result.response.understood.targetCity, city); assert.equal(result.response.candidates.length, 5); }
  const alternative = await executeFounderLiveDecision(request("alt", "Café in Basel", { alternativeRequested: true }), ACTOR, ports()); assert.equal(alternative.response.alternative.requested, true); assert.equal(alternative.response.alternative.negativeSignalProduced, false);
  ...
  const replay = await executeFounderLiveDecision(request("replay", "Café in Basel"), ACTOR, ports()); const rebuilt = await executeFounderLiveDecision(request("replay", "Café in Basel"), ACTOR, ports()); assert.equal(canonicalJson(replay), canonicalJson(rebuilt));
});

test("client authority injection, bad auth, versions, locations, limits and kill switch fail closed", async () => {
  await assert.rejects(() => executeFounderLiveDecision({ ...request("inject", "Café in Basel"), userId: "other" }, ACTOR, ports()));
  await assert.rejects(() => executeFounderLiveDecision({ ...request("version", "Café in Basel"), contractVersion: "future" }, ACTOR, ports()));
  ...
  await assert.rejects(() => executeFounderLiveDecision(request("scope", "Café in Basel"), ACTOR, mismatch), /LOCATION_AUTHORITY_MISMATCH/);
  const killed = ports(); killed.state.killed = true; await assert.rejects(() => executeFounderLiveDecision(request("killed", "Café in Basel"), ACTOR, killed), /KILL_SWITCH_ENGAGED/);
  ...
  const notAllowed = createFounderLiveHttpHandler(ports({ allowlist: { contractVersion: "backyrd.decision-vnext.founder-live-allowlist-port@1.0", async authorize() { return false; } } }));
  ...
  const idemPorts = ports(); await executeFounderLiveDecision(request("same", "Café in Basel"), ACTOR, idemPorts);
  await assert.rejects(() => executeFounderLiveDecision({ ...request("same", "Bar in Basel"), requestId: "request-same-two" }, ACTOR, idemPorts), /IDEMPOTENCY_CONFLICT/);
});
...
test("missing user projection degrades neutrally, port failures close, timeout is bounded and no deploy is claimed", async () => {
  const neutral = await executeFounderLiveDecision(request("neutral", "Café in Basel"), ACTOR, ports()); assert.equal(neutral.expert.provenance.userProjectionHash.length, 64);
  ...
  await assert.rejects(() => executeFounderLiveDecision(request("world-down", "Café in Basel"), ACTOR, brokenWorld));
...
test("dual run is deterministic, local/prod-like only and cannot produce Product output or learning", async () => {
  ...
  const one = await executeFounderLiveDualRun({ request: value, actor: ACTOR, primary: ports(), comparator: ports() });
  const two = await executeFounderLiveDualRun({ request: value, actor: ACTOR, primary: ports(), comparator: ports() });
...
test("default OFF and evaluator version drift fail closed before World evaluation", async () => {
  ...
  await assert.rejects(() => executeFounderLiveDecision(request("off", "Café in Basel"), ACTOR, disabled), /API_DISABLED/);
  ...
  await assert.rejects(() => executeFounderLiveDecision(request("drift", "Café in Basel"), ACTOR, drift), /SERVER_PORT_VERSION_UNSUPPORTED/);
});
