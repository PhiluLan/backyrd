...
export function createSyntheticExecution(input: { request: unknown; world: SyntheticWorld; baseline: BaselineId; sourceSha: string; authorizedLocationScope: { readonly kind: "city"; readonly city: string }; candidatePoolSize?: number; actor?: { kind: "user"; userId: string; subjectBindingHash: string } | { kind: "anonymous"; subjectBindingHash: string }; personalizationKillSwitch?: boolean }): DecisionExecutionEnvelope {
  ...
  if (!input.authorizedLocationScope) throw new Error("context_authorized_location_scope_required");
  const context = resolveSituationalContext(request, { decisionId, sessionId, executedAt: input.world.observedAt, actorSubjectBindingHash: actor.subjectBindingHash, authorizedLocationScope: input.authorizedLocationScope });
