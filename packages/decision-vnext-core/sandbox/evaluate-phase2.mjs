...
import {
  ...
  canonicalJson, contentHash, createSyntheticEvaluationAuthority, generateSyntheticWorld, replayPhase2Evaluation, runPhase2Evaluation,
  trustSyntheticEvaluationAuthorityForLocalExecution,
} from "../dist/index.js";
...
const sourceTreeHash = execFileSync("git", ["rev-parse", "HEAD^{tree}"], { encoding: "utf8" }).trim();
const artifactIdentityHash = contentHash({ package: "@backyrd/decision-vnext-core", sourceSha, sourceTreeHash, artifactContract: "phase2-local-ci-artifact-v1" });
...
const evaluationAuthority = createSyntheticEvaluationAuthority({ authorityId: `phase2-authority-${config.seed}`, scenarioId: `phase2-seed-${config.seed}`, sandboxConfig: config, worldHash: world.worldHash, sourceSha, sourceTreeHash, artifactIdentityHash });
...
const { envelope, report } = await runPhase2Evaluation({
  evaluationAuthority, request, world,
