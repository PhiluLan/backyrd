...
export const PHASE3D_CALIBRATION_CANDIDATES = Object.freeze([
  candidate("CONSERVATIVE", {
    ...
    skipMaturity: { independentJourneys: 8, stableTargetRequired: true, corroborationRequired: true, globalAversionForbidden: true },
  ...
  candidate("BALANCED", {
    ...
    skipMaturity: { independentJourneys: 5, stableTargetRequired: true, corroborationRequired: true, globalAversionForbidden: true },
...
export const Phase3DCalibrationTrustAnchorSchema = schema.object({ contractVersion: schema.literal(CONTRACT_VERSIONS.phase3dCalibrationTrustAnchor), anchorId: identifier, acceptedReleaseId: identifier, acceptedReleaseHash: sha256, acceptedCandidateSetHash: sha256, acceptedProductPolicyHash: sha256, acceptedRegistryHash: sha256, issuer: schema.literal("BACKYRD_CTO_EVALUATION_TRUST_REGISTRY"), environment: schema.literal("LOCAL_FOUNDER_LAB"), validFrom: timestamp, validUntil: timestamp, productionAuthorized: schema.literal(false), anchorHash: sha256 });
const anchorBody = { contractVersion: CONTRACT_VERSIONS.phase3dCalibrationTrustAnchor, anchorId: "backyrd-user-intelligence-phase3d-calibration-anchor-1", acceptedReleaseId: PHASE3D_CALIBRATION_RELEASE.releaseId, acceptedReleaseHash: PHASE3D_CALIBRATION_RELEASE.releaseHash, acceptedCandidateSetHash: PHASE3D_CANDIDATE_SET_HASH, acceptedProductPolicyHash: PRODUCT_INTERPRETATION_POLICY_3C.policyHash, acceptedRegistryHash: PRODUCT_SIGNAL_SEMANTICS_REGISTRY_3C.registryHash, issuer: "BACKYRD_CTO_EVALUATION_TRUST_REGISTRY" as const, environment: "LOCAL_FOUNDER_LAB" as const, validFrom: "2026-09-12T00:00:00.000Z", validUntil: "2030-01-01T00:00:00.000Z", productionAuthorized: false as const };
...
export function verifyPhase3DCalibrationCandidates(values: unknown, trust: Phase3DCalibrationTrustContext): readonly Phase3DCalibrationCandidate[] {
  ...
  if (release.candidateSetHash !== contentHash(rows) || anchor.acceptedCandidateSetHash !== contentHash(rows) || anchor.acceptedReleaseHash !== release.releaseHash) throw new ContractValidationError("$.authority", "candidate authority binding mismatch");
  return rows;
}
...
export const PHASE3D_SCENARIO_IDS = Object.freeze([
  ...
  "skip-single", "skip-same-journey", "skip-different-decisions", "skip-stable-target", "skip-time-pressure", "skip-then-positive", "spot-not-fit", "alternative-neutral",
...
export const PHASE3D_SCENARIO_SET_VERSION = "backyrd.user-intelligence.founder-lab-scenarios@3d-1";
export const PHASE3D_SCENARIO_SET_HASH = contentHash({ version: PHASE3D_SCENARIO_SET_VERSION, ids: PHASE3D_SCENARIO_IDS });

const CandidateOutcomeSchema = schema.object({ candidateId: schema.enum(PHASE3D_CANDIDATE_IDS), searchContextualTargets: schema.array(identifier, { max: 256 }), searchLongTermTargets: schema.array(identifier, { max: 256 }), maturedSkipTargets: schema.array(identifier, { max: 256 }), conceptTasteTargets: schema.array(identifier, { max: 256 }), retentionDecisionRequired: schema.literal(true), productionProjectionAuthorized: schema.literal(false), outcomeHash: sha256 });
...
function candidateOutcome(candidate: Phase3DCalibrationCandidate, rows: readonly ProductPolicyObservation[]): Infer<typeof CandidateOutcomeSchema> {
  ...
  const maturedSkipTargets = [...skips].filter(([, list]) => list.length >= candidate.rules.skipMaturity.independentJourneys && !rows.some((row) => row.active && row.eventType === "EXPLICIT_SATISFACTION" && row.spotId === list[0]!.spotId && row.contextHash === list[0]!.contextHash) && (!candidate.rules.skipMaturity.corroborationRequired || rows.some(({ eventType }) => eventType === "SPOT_NOT_FIT"))).map(([key]) => `contextual-skip:${contentHash(key)}`).sort();
  ...
  const body = { candidateId: candidate.candidateId, searchContextualTargets, searchLongTermTargets, maturedSkipTargets, conceptTasteTargets, retentionDecisionRequired: true as const, productionProjectionAuthorized: false as const };
...
export function buildPhase3DFounderLabReport(input: ProductPolicyEvaluationInput, phase3cRelease: Phase3CReleaseTrustContext, evidenceTrust: Phase3CEvidenceTrustContext, calibrationTrust: Phase3DCalibrationTrustContext, generatedAt = "2026-09-12T12:00:00.000Z"): Phase3DFounderLabReport {
  ...
  const outcomes = candidates.map((item) => candidateOutcome(item, rows));
...
export function verifyPhase3DFounderLabReport(value: unknown, input: ProductPolicyEvaluationInput, phase3cRelease: Phase3CReleaseTrustContext, evidenceTrust: Phase3CEvidenceTrustContext, calibrationTrust: Phase3DCalibrationTrustContext): Phase3DFounderLabReport {
  ...
  const rebuilt = buildPhase3DFounderLabReport(input, phase3cRelease, evidenceTrust, calibrationTrust, parsed.generatedAt); if (!same(parsed, rebuilt)) throw new ContractValidationError("$", "lab report differs from authoritative reconstruction"); return parsed;
}
...
export const PHASE3D_RELEASE_SUMMARY_BODY = Object.freeze({ contractVersion: CONTRACT_VERSIONS.phase3dReleaseSummary, generatorVersion: "backyrd.user-intelligence.phase3d-report-generator@1.0", scenarioSetVersion: PHASE3D_SCENARIO_SET_VERSION, scenarioSetHash: PHASE3D_SCENARIO_SET_HASH, scenarioCount: PHASE3D_SCENARIO_IDS.length, candidateSetHash: PHASE3D_CANDIDATE_SET_HASH, candidateHashes: PHASE3D_CALIBRATION_CANDIDATES.map(({ candidateId, candidateHash }) => ({ candidateId, candidateHash })), sourceProductPolicyHash: PRODUCT_INTERPRETATION_POLICY_3C.policyHash, sourceRegistryHash: PRODUCT_SIGNAL_SEMANTICS_REGISTRY_3C.registryHash, calibrationReleaseHash: PHASE3D_CALIBRATION_RELEASE.releaseHash, calibrationTrustAnchorHash: PHASE3D_CALIBRATION_TRUST_ANCHOR.anchorHash, fullReportHash: "GENERATED_BY_RELEASE_SCRIPT", productionAuthorized: false as const, runtimeActivated: false as const, rankingAuthorized: false as const, eligibilityAuthorized: false as const, executionAuthorized: false as const });
