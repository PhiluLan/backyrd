...
export const PHASE3D_SCENARIO_IDS = Object.freeze([
  "search-single-intent", "search-repeated-context", "search-travel-one-off", "search-for-other-person", "search-duplicate-retry", "search-same-journey", "search-negative-outcome", "search-context-flip", "search-uncertain-world", "search-without-visit", "search-positive-experience", "search-contextual-2-withheld", "search-contextual-3-pass", "search-long-term-5-withheld", "search-long-term-6-pass",
  ...
  "concept-one-spot-many-concepts", "concept-same-spot-repeated", "concept-distinct-spots", "concept-two-spots-withheld", "concept-three-spots-pass", "concept-context-split", "concept-uncertain-world", "concept-conflict", "concept-corrected", "concept-different-targets",
  "retention-purpose-tiered", "retention-durations-unconfigured", "dwell-attention-only", "three-independent-visits", "full-incremental-parity", "consent-withdrawal", "full-reset", "account-erasure", "commercial-counterfactual", "authority-tampering",
  "ui-search-pattern", "ui-founder-selections", "ui-skip-thresholds", "ui-decision-task-select", "ui-familiarity", "ui-satisfaction", "ui-negative-situation", "ui-conflict", "ui-dwell", "ui-correction-retry", "ui-lifecycle", "ui-reload", "ui-narrow-viewport",
] as const);
export const PHASE3D_SCENARIO_SET_VERSION = "backyrd.user-intelligence.founder-lab-scenarios@3d-4";
...
const finalSelectionStatus = schema.object({ founder: schema.literal("FOUNDER_SELECTED"), calibration: schema.literal("CALIBRATION_APPROVED"), activation: schema.literal("PRODUCT_ACTIVATION_NOT_AUTHORIZED") });
const finalSelection = { founder: "FOUNDER_SELECTED" as const, calibration: "CALIBRATION_APPROVED" as const, activation: "PRODUCT_ACTIVATION_NOT_AUTHORIZED" as const };
export const Phase3DFounderCalibrationDecisionRecordSchema = schema.object({
  contractVersion: schema.literal(CONTRACT_VERSIONS.phase3dFounderCalibrationDecisionRecord), recordId: identifier,
  previousRecordId: schema.literal(PHASE3D_CALIBRATION_DECISION_RECORD.recordId), previousRecordHash: schema.literal(PHASE3D_CALIBRATION_DECISION_RECORD.recordHash), authority: schema.literal("CALIBRATION_ONLY"),
  decisions: schema.object({
    searchContextualMaturity: schema.object({ status: finalSelectionStatus, independentJourneys: schema.literal(3), semanticMatchRequired: schema.literal(true), contextPreserved: schema.literal(true), rawTextStored: schema.literal(false), longTermTransfer: schema.literal(false) }),
    searchLongTermPromotion: schema.object({ status: finalSelectionStatus, independentJourneys: schema.literal(6), selfSearchOnly: schema.literal(true), oneOffTravelExcluded: schema.literal(true), otherPersonExcluded: schema.literal(true), minimizedIdsOnly: schema.literal(true), contextPreserved: schema.literal(true), contextlessTasteForbidden: schema.literal(true), conflictsPreserved: schema.literal(true), automaticDecay: schema.literal(false) }),
    skipMaturity: schema.object({ status: finalSelectionStatus, independentJourneys: schema.literal(3), target: schema.literal("SUBJECT_SPOT_DECISION_TASK_CONTEXT"), oneActivePerJourney: schema.literal(true), correctedOrInactiveExcluded: schema.literal(true), globalSpotAversionForbidden: schema.literal(true), conceptAversionForbidden: schema.literal(true), rankingAuthorized: schema.literal(false), eligibilityAuthorized: schema.literal(false) }),
    conceptTasteMinimumDistinctSpots: schema.object({ status: finalSelectionStatus, distinctSpots: schema.literal(3), independentQualifiedExperiences: schema.literal(3), explicitDirectedOutcomeRequired: schema.literal(true), certainEventTimeWorldAttributionRequired: schema.literal(true), oneAttributionUnitPerExperience: schema.literal(true), conflictsPreserved: schema.literal(true), strongCertaintyForbidden: schema.literal(true) }),
    retention: schema.object({ status: finalSelectionStatus, strategy: schema.literal("PURPOSE_TIERED_RETENTION"), classes: schema.array(schema.enum(["SEARCH_MINIMIZED", "SKIP_CONTEXTUAL", "CONTEXT_MINIMIZED", "EVIDENCE_LEDGER", "USER_MODEL_DERIVED", "CORRECTION_GUARD", "NON_PERSONAL_AUDIT"] as const), { min: 7, max: 7 }), durations: schema.literal("NOT_CONFIGURED_PENDING_LEGAL_AND_CTO_APPROVAL"), pseudonymousRemainsPersonal: schema.literal(true), withdrawalRemovesDerivedPersonalization: schema.literal(true), resetDeletesModelProjectionCacheAndRebuild: schema.literal(true), erasureDeletesAllPersonalStores: schema.literal(true), nonPersonalAuditMustBeNonReidentifiable: schema.literal(true) }),
  }),
  productionAuthorized: schema.literal(false), runtimeActivated: schema.literal(false), rankingAuthorized: schema.literal(false), eligibilityAuthorized: schema.literal(false), shadowTrafficAuthorized: schema.literal(false), recordHash: sha256,
});
const founderDecisionBody = { contractVersion: CONTRACT_VERSIONS.phase3dFounderCalibrationDecisionRecord, recordId: "backyrd-user-intelligence-phase3d-founder-calibration-decisions-2", previousRecordId: PHASE3D_CALIBRATION_DECISION_RECORD.recordId, previousRecordHash: PHASE3D_CALIBRATION_DECISION_RECORD.recordHash, authority: "CALIBRATION_ONLY" as const, decisions: {
  searchContextualMaturity: { status: finalSelection, independentJourneys: 3 as const, semanticMatchRequired: true as const, contextPreserved: true as const, rawTextStored: false as const, longTermTransfer: false as const },
  searchLongTermPromotion: { status: finalSelection, independentJourneys: 6 as const, selfSearchOnly: true as const, oneOffTravelExcluded: true as const, otherPersonExcluded: true as const, minimizedIdsOnly: true as const, contextPreserved: true as const, contextlessTasteForbidden: true as const, conflictsPreserved: true as const, automaticDecay: false as const },
  skipMaturity: { status: finalSelection, independentJourneys: 3 as const, target: "SUBJECT_SPOT_DECISION_TASK_CONTEXT" as const, oneActivePerJourney: true as const, correctedOrInactiveExcluded: true as const, globalSpotAversionForbidden: true as const, conceptAversionForbidden: true as const, rankingAuthorized: false as const, eligibilityAuthorized: false as const },
  conceptTasteMinimumDistinctSpots: { status: finalSelection, distinctSpots: 3 as const, independentQualifiedExperiences: 3 as const, explicitDirectedOutcomeRequired: true as const, certainEventTimeWorldAttributionRequired: true as const, oneAttributionUnitPerExperience: true as const, conflictsPreserved: true as const, strongCertaintyForbidden: true as const },
  retention: { status: finalSelection, strategy: "PURPOSE_TIERED_RETENTION" as const, classes: ["SEARCH_MINIMIZED", "SKIP_CONTEXTUAL", "CONTEXT_MINIMIZED", "EVIDENCE_LEDGER", "USER_MODEL_DERIVED", "CORRECTION_GUARD", "NON_PERSONAL_AUDIT"] as const, durations: "NOT_CONFIGURED_PENDING_LEGAL_AND_CTO_APPROVAL" as const, pseudonymousRemainsPersonal: true as const, withdrawalRemovesDerivedPersonalization: true as const, resetDeletesModelProjectionCacheAndRebuild: true as const, erasureDeletesAllPersonalStores: true as const, nonPersonalAuditMustBeNonReidentifiable: true as const },
}, productionAuthorized: false as const, runtimeActivated: false as const, rankingAuthorized: false as const, eligibilityAuthorized: false as const, shadowTrafficAuthorized: false as const };
export const PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD = Phase3DFounderCalibrationDecisionRecordSchema.parse({ ...founderDecisionBody, recordHash: contentHash(founderDecisionBody) });
export function verifyPhase3DFounderCalibrationDecisionRecord(value: unknown) {
  const parsed = Phase3DFounderCalibrationDecisionRecordSchema.parse(value);
  if (contentHash(without(parsed as unknown as Record<string, unknown>, "recordHash")) !== parsed.recordHash || !same(parsed, PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD)) throw new ContractValidationError("$.founderCalibrationDecisionRecord", "record is not the accepted append-only Founder calibration decision");
  return parsed;
}

export const PHASE3D_CALIBRATION_DECISION_IDS = Object.freeze(["SEARCH_CONTEXTUAL_MATURITY", "SEARCH_LONG_TERM_PROMOTION", "SKIP_MATURITY", "CONCEPT_TASTE_MINIMUM_DISTINCT_SPOTS", "RETENTION"] as const);
const TraceRowSchema = schema.object({ decisionId: schema.enum(PHASE3D_CALIBRATION_DECISION_IDS), policyRule: identifier, registryEntries: schema.array(identifier, { min: 1, max: 12 }), reducerBehavior: identifier, sufficiencyOrMaturity: identifier, projectionBoundary: identifier, lifecycleOrRetention: identifier, positiveTests: schema.array(identifier, { min: 1, max: 12 }), negativeTests: schema.array(identifier, { min: 1, max: 12 }) });
export const Phase3DCalibrationTraceabilitySchema = schema.object({ contractVersion: schema.literal(CONTRACT_VERSIONS.phase3dCalibrationTraceability), decisionRecordHash: sha256, rows: schema.array(TraceRowSchema, { min: 5, max: 5 }), traceabilityHash: sha256 });
const traceRows = [
  { decisionId: "SEARCH_CONTEXTUAL_MATURITY" as const, policyRule: "phase3d.search.contextual-maturity", registryEntries: ["SEARCH"], reducerBehavior: "GROUP_ACTIVE_MINIMIZED_SEARCH_BY_CONCEPT_AND_CONTEXT", sufficiencyOrMaturity: "THREE_INDEPENDENT_SEMANTIC_JOURNEYS", projectionBoundary: "CALIBRATION_HYPOTHESIS_CONTEXT_BOUND_ONLY", lifecycleOrRetention: "SEARCH_MINIMIZED_PURPOSE_TIER", positiveTests: ["search-contextual-3-pass"], negativeTests: ["search-contextual-2-withheld", "search-same-journey"] },
  { decisionId: "SEARCH_LONG_TERM_PROMOTION" as const, policyRule: "phase3d.search.long-term-readiness", registryEntries: ["SEARCH"], reducerBehavior: "GROUP_SELF_SEARCH_WITH_CONTEXT_PRESERVED", sufficiencyOrMaturity: "SIX_INDEPENDENT_SEMANTIC_JOURNEYS", projectionBoundary: "CALIBRATION_READINESS_NO_CONTEXTLESS_TASTE", lifecycleOrRetention: "SEARCH_MINIMIZED_PURPOSE_TIER", positiveTests: ["search-long-term-6-pass"], negativeTests: ["search-long-term-5-withheld", "search-for-other-person", "search-travel-one-off"] },
  { decisionId: "SKIP_MATURITY" as const, policyRule: "phase3d.skip.contextual-maturity", registryEntries: ["QUICK_SKIP"], reducerBehavior: "GROUP_ONE_ACTIVE_SKIP_PER_JOURNEY_BY_SUBJECT_SPOT_TASK_CONTEXT", sufficiencyOrMaturity: "THREE_INDEPENDENT_MATCHING_JOURNEYS", projectionBoundary: "WEAK_CALIBRATION_ONLY_NO_GLOBAL_AVERSION", lifecycleOrRetention: "SKIP_PURPOSE_TIER", positiveTests: ["skip-three-threshold", "skip-five-threshold", "skip-eight-threshold"], negativeTests: ["skip-two-threshold", "skip-same-journey", "skip-task-split", "skip-context-split", "skip-spot-split", "skip-inactive-corrected"] },
  { decisionId: "CONCEPT_TASTE_MINIMUM_DISTINCT_SPOTS" as const, policyRule: "phase3d.concept.minimum-distinct-spots", registryEntries: ["EXPLICIT_SATISFACTION", "EXPLICIT_DISSATISFACTION"], reducerBehavior: "COUNT_DISTINCT_SPOTS_WITH_DIRECTED_OUTCOME_AND_CERTAIN_WORLD_ATTRIBUTION", sufficiencyOrMaturity: "THREE_INDEPENDENT_QUALIFIED_EXPERIENCES_AT_DISTINCT_SPOTS", projectionBoundary: "LIMITED_CALIBRATION_HYPOTHESIS_ONLY", lifecycleOrRetention: "EVIDENCE_AND_MODEL_PURPOSE_TIERS", positiveTests: ["concept-three-spots-pass"], negativeTests: ["concept-two-spots-withheld", "concept-uncertain-world", "concept-same-spot-repeated"] },
  { decisionId: "RETENTION" as const, policyRule: "phase3d.retention.purpose-tiered", registryEntries: ["SEARCH", "QUICK_SKIP", "DWELL", "CORRECTION"], reducerBehavior: "SEPARATE_RETENTION_CLASSES_WITHOUT_DURATION_DEFAULT", sufficiencyOrMaturity: "NOT_APPLICABLE", projectionBoundary: "NO_RAW_OR_RETENTION_METADATA_IN_DECISION_PROJECTION", lifecycleOrRetention: "PURPOSE_TIERED_DURATIONS_NOT_CONFIGURED", positiveTests: ["retention-purpose-tiered"], negativeTests: ["retention-durations-unconfigured", "authority-tampering"] },
];
const traceBody = { contractVersion: CONTRACT_VERSIONS.phase3dCalibrationTraceability, decisionRecordHash: PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD.recordHash, rows: traceRows };
export const PHASE3D_CALIBRATION_TRACEABILITY = Phase3DCalibrationTraceabilitySchema.parse({ ...traceBody, traceabilityHash: contentHash(traceBody) });

export const Phase3DCalibrationReleaseSchema = schema.object({ contractVersion: schema.literal(CONTRACT_VERSIONS.phase3dCalibrationReleaseFinal), releaseId: identifier, candidateSetHash: sha256, previousCalibrationDecisionRecordHash: sha256, calibrationDecisionRecordHash: sha256, calibrationTraceabilityHash: sha256, scenarioSetHash: sha256, decisionTaskRegistryHash: sha256, sourceProductPolicyHash: sha256, sourceRegistryHash: sha256, issuer: schema.literal("BACKYRD_USER_INTELLIGENCE_EVALUATION_RELEASE"), status: schema.literal("FOUNDER_EVALUATION_ONLY"), validFrom: timestamp, validUntil: timestamp, productionAuthorized: schema.literal(false), runtimeActivated: schema.literal(false), rankingAuthorized: schema.literal(false), eligibilityAuthorized: schema.literal(false), shadowTrafficAuthorized: schema.literal(false), releaseHash: sha256 });
const releaseBody = { contractVersion: CONTRACT_VERSIONS.phase3dCalibrationReleaseFinal, releaseId: "backyrd-user-intelligence-phase3d-calibration-release-3", candidateSetHash: PHASE3D_CANDIDATE_SET_HASH, previousCalibrationDecisionRecordHash: PHASE3D_CALIBRATION_DECISION_RECORD.recordHash, calibrationDecisionRecordHash: PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD.recordHash, calibrationTraceabilityHash: PHASE3D_CALIBRATION_TRACEABILITY.traceabilityHash, scenarioSetHash: PHASE3D_SCENARIO_SET_HASH, decisionTaskRegistryHash: PHASE3D_DECISION_TASK_REGISTRY_HASH, sourceProductPolicyHash: PRODUCT_INTERPRETATION_POLICY_3C.policyHash, sourceRegistryHash: PRODUCT_SIGNAL_SEMANTICS_REGISTRY_3C.registryHash, issuer: "BACKYRD_USER_INTELLIGENCE_EVALUATION_RELEASE" as const, status: "FOUNDER_EVALUATION_ONLY" as const, validFrom: "2026-09-12T00:00:00.000Z", validUntil: "2030-01-01T00:00:00.000Z", productionAuthorized: false as const, runtimeActivated: false as const, rankingAuthorized: false as const, eligibilityAuthorized: false as const, shadowTrafficAuthorized: false as const };
...
export const Phase3DCalibrationTrustAnchorSchema = schema.object({ contractVersion: schema.literal(CONTRACT_VERSIONS.phase3dCalibrationTrustAnchorFinal), anchorId: identifier, acceptedReleaseId: identifier, acceptedReleaseHash: sha256, acceptedCandidateSetHash: sha256, acceptedPreviousCalibrationDecisionRecordHash: sha256, acceptedCalibrationDecisionRecordHash: sha256, acceptedCalibrationTraceabilityHash: sha256, acceptedScenarioSetHash: sha256, acceptedDecisionTaskRegistryHash: sha256, acceptedProductPolicyHash: sha256, acceptedRegistryHash: sha256, issuer: schema.literal("BACKYRD_CTO_EVALUATION_TRUST_REGISTRY"), environment: schema.literal("LOCAL_FOUNDER_LAB"), validFrom: timestamp, validUntil: timestamp, productionAuthorized: schema.literal(false), runtimeActivated: schema.literal(false), rankingAuthorized: schema.literal(false), eligibilityAuthorized: schema.literal(false), shadowTrafficAuthorized: schema.literal(false), anchorHash: sha256 });
const anchorBody = { contractVersion: CONTRACT_VERSIONS.phase3dCalibrationTrustAnchorFinal, anchorId: "backyrd-user-intelligence-phase3d-calibration-anchor-3", acceptedReleaseId: PHASE3D_CALIBRATION_RELEASE.releaseId, acceptedReleaseHash: PHASE3D_CALIBRATION_RELEASE.releaseHash, acceptedCandidateSetHash: PHASE3D_CANDIDATE_SET_HASH, acceptedPreviousCalibrationDecisionRecordHash: PHASE3D_CALIBRATION_DECISION_RECORD.recordHash, acceptedCalibrationDecisionRecordHash: PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD.recordHash, acceptedCalibrationTraceabilityHash: PHASE3D_CALIBRATION_TRACEABILITY.traceabilityHash, acceptedScenarioSetHash: PHASE3D_SCENARIO_SET_HASH, acceptedDecisionTaskRegistryHash: PHASE3D_DECISION_TASK_REGISTRY_HASH, acceptedProductPolicyHash: PRODUCT_INTERPRETATION_POLICY_3C.policyHash, acceptedRegistryHash: PRODUCT_SIGNAL_SEMANTICS_REGISTRY_3C.registryHash, issuer: "BACKYRD_CTO_EVALUATION_TRUST_REGISTRY" as const, environment: "LOCAL_FOUNDER_LAB" as const, validFrom: "2026-09-12T00:00:00.000Z", validUntil: "2030-01-01T00:00:00.000Z", productionAuthorized: false as const, runtimeActivated: false as const, rankingAuthorized: false as const, eligibilityAuthorized: false as const, shadowTrafficAuthorized: false as const };
...
export function verifyPhase3DCalibrationCandidates(values: unknown, trust: Phase3DCalibrationTrustContext): readonly Phase3DCalibrationCandidate[] {
  verifyPhase3DFounderCalibrationDecisionRecord(PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD);
  ...
  if (release.candidateSetHash !== contentHash(rows) || anchor.acceptedCandidateSetHash !== contentHash(rows) || anchor.acceptedReleaseHash !== release.releaseHash || release.previousCalibrationDecisionRecordHash !== PHASE3D_CALIBRATION_DECISION_RECORD.recordHash || anchor.acceptedPreviousCalibrationDecisionRecordHash !== PHASE3D_CALIBRATION_DECISION_RECORD.recordHash || release.calibrationDecisionRecordHash !== PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD.recordHash || anchor.acceptedCalibrationDecisionRecordHash !== PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD.recordHash || release.calibrationTraceabilityHash !== PHASE3D_CALIBRATION_TRACEABILITY.traceabilityHash || anchor.acceptedCalibrationTraceabilityHash !== PHASE3D_CALIBRATION_TRACEABILITY.traceabilityHash || release.scenarioSetHash !== PHASE3D_SCENARIO_SET_HASH || anchor.acceptedScenarioSetHash !== PHASE3D_SCENARIO_SET_HASH || release.decisionTaskRegistryHash !== PHASE3D_DECISION_TASK_REGISTRY_HASH || anchor.acceptedDecisionTaskRegistryHash !== PHASE3D_DECISION_TASK_REGISTRY_HASH) throw new ContractValidationError("$.authority", "candidate authority binding mismatch");
  return rows;
}
...
const FounderSelectionOutcomeSchema = schema.object({ searchContextualTargets: schema.array(identifier, { max: 256 }), searchLongTermTargets: schema.array(identifier, { max: 256 }), maturedSkipTargets: schema.array(identifier, { max: 256 }), conceptTasteTargets: schema.array(identifier, { max: 256 }), retentionStrategy: schema.literal("PURPOSE_TIERED_RETENTION"), retentionDurations: schema.literal("NOT_CONFIGURED_PENDING_LEGAL_AND_CTO_APPROVAL"), productionProjectionAuthorized: schema.literal(false), rankingAuthorized: schema.literal(false), eligibilityAuthorized: schema.literal(false), outcomeHash: sha256 });
...
export const Phase3DFounderLabReportSchema = schema.object({ contractVersion: schema.literal(CONTRACT_VERSIONS.phase3dReport), reportId: identifier, generatedAt: timestamp, mode: schema.literal("FOUNDER_EVALUATION_ONLY"), localOnly: schema.literal(true), productionAuthorized: schema.literal(false), rankingAuthority: schema.literal(false), eligibilityAuthority: schema.literal(false), sourceStateHash: schema.nullable(sha256), subjectBindingHash: schema.nullable(sha256), lifecycle: schema.enum(["ACTIVE", "NO_CONSENT", "WITHDRAWN", "RESET", "ERASED"] as const), scenarioSetVersion: schema.literal(PHASE3D_SCENARIO_SET_VERSION), scenarioSetHash: sha256, candidateSetHash: sha256, candidateOutcomes: schema.array(CandidateOutcomeSchema, { min: 3, max: 3 }), founderSelectionOutcome: FounderSelectionOutcomeSchema, metrics: MetricSchema, primaryView: schema.array(FriendlyItemSchema, { max: 4096 }), expert: schema.object({ productPolicyHash: sha256, registryHash: sha256, phase3cReleaseArtifactHash: sha256, previousCalibrationDecisionRecordHash: sha256, calibrationDecisionRecordHash: sha256, calibrationTraceabilityHash: sha256, calibrationReleaseHash: sha256, calibrationAnchorHash: sha256, observationHashes: schema.array(sha256, { max: 4096 }), decisionTaskBindingHashes: schema.array(sha256, { max: 4096 }), reasonCodes: schema.array(identifier, { max: 4096 }) }), neutralProductionProjectionHash: sha256, limitations: schema.array(identifier, { min: 1, max: 32 }), reportHash: sha256 });
...
function founderSelectionOutcome(rows: readonly ProductPolicyObservation[], decisionTaskBindings: readonly Phase3DDecisionTaskBinding[]): Infer<typeof FounderSelectionOutcomeSchema> {
  const searches = groupedSearches(rows); const skips = groupedSkips(rows, decisionTaskBindings);
  const hasNegativeOutcome = (conceptId: string, contextHash: string | null) => rows.some((row) => row.active && row.eventType === "EXPLICIT_DISSATISFACTION" && row.contextHash === contextHash && row.conceptIds.includes(conceptId));
  const selectedSearchTargets = (minimum: number, prefix: string) => [...searches].filter(([key, list]) => list.length >= minimum && list.every(({ worldAttribution }) => worldAttribution === "CERTAIN") && !hasNegativeOutcome(key.split("|")[0]!, list[0]!.contextHash)).map(([key]) => `${prefix}:${contentHash(key)}`).sort();
  const maturedSkipTargets = skips.filter(({ binding, rows: list }) => list.length >= 3 && !rows.some((row) => row.active && row.eventType === "EXPLICIT_SATISFACTION" && row.spotId === binding.spotId && row.contextHash === binding.contextHash)).map(({ semanticTargetHash }) => `contextual-skip:${semanticTargetHash}`).sort();
  const conceptSpots = new Map<string, Set<string>>(); for (const row of rows.filter((item) => item.active && item.eventType === "EXPLICIT_SATISFACTION" && item.worldAttribution === "CERTAIN" && rowHasIndependentJourney(item))) for (const id of row.conceptIds) { const set = conceptSpots.get(id) ?? new Set<string>(); if (row.spotId) set.add(row.spotId); conceptSpots.set(id, set); }
  const conceptTasteTargets = [...conceptSpots].filter(([id, spots]) => spots.size >= 3 && !rows.some((row) => row.active && row.eventType === "EXPLICIT_DISSATISFACTION" && row.conceptIds.includes(id))).map(([id]) => `concept:${id}`).sort();
  const body = { searchContextualTargets: selectedSearchTargets(3, "founder-contextual-search"), searchLongTermTargets: selectedSearchTargets(6, "founder-long-term-search"), maturedSkipTargets, conceptTasteTargets, retentionStrategy: "PURPOSE_TIERED_RETENTION" as const, retentionDurations: "NOT_CONFIGURED_PENDING_LEGAL_AND_CTO_APPROVAL" as const, productionProjectionAuthorized: false as const, rankingAuthorized: false as const, eligibilityAuthorized: false as const };
  return FounderSelectionOutcomeSchema.parse({ ...body, outcomeHash: contentHash(body) });
}
...
export function buildPhase3DFounderLabReport(input: ProductPolicyEvaluationInput, phase3cRelease: Phase3CReleaseTrustContext, evidenceTrust: Phase3CEvidenceTrustContext, calibrationTrust: Phase3DCalibrationTrustContext, generatedAt = "2026-09-12T12:00:00.000Z", options: Phase3DReportOptions = {}): Phase3DFounderLabReport {
  ...
  const founderOutcome = founderSelectionOutcome(rows, decisionTaskBindings);
...
export function verifyPhase3DFounderLabReport(value: unknown, input: ProductPolicyEvaluationInput, phase3cRelease: Phase3CReleaseTrustContext, evidenceTrust: Phase3CEvidenceTrustContext, calibrationTrust: Phase3DCalibrationTrustContext, options: Phase3DReportOptions = {}): Phase3DFounderLabReport {
  ...
  if (contentHash(without(parsed.founderSelectionOutcome as unknown as Record<string, unknown>, "outcomeHash")) !== parsed.founderSelectionOutcome.outcomeHash) throw new ContractValidationError("$.founderSelectionOutcome", "Founder selection outcome hash mismatch");
...
export const PHASE3D_RELEASE_SUMMARY_BODY = Object.freeze({ contractVersion: CONTRACT_VERSIONS.phase3dReleaseSummary, generatorVersion: "backyrd.user-intelligence.phase3d-report-generator@1.3", scenarioSetVersion: PHASE3D_SCENARIO_SET_VERSION, scenarioSetHash: PHASE3D_SCENARIO_SET_HASH, scenarioCount: PHASE3D_SCENARIO_IDS.length, candidateSetHash: PHASE3D_CANDIDATE_SET_HASH, candidateHashes: PHASE3D_CALIBRATION_CANDIDATES.map(({ candidateId, candidateHash }) => ({ candidateId, candidateHash })), previousCalibrationDecisionRecordHash: PHASE3D_CALIBRATION_DECISION_RECORD.recordHash, calibrationDecisionRecordHash: PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD.recordHash, calibrationTraceabilityHash: PHASE3D_CALIBRATION_TRACEABILITY.traceabilityHash, decisionTaskRegistryHash: PHASE3D_DECISION_TASK_REGISTRY_HASH, sourceProductPolicyHash: PRODUCT_INTERPRETATION_POLICY_3C.policyHash, sourceRegistryHash: PRODUCT_SIGNAL_SEMANTICS_REGISTRY_3C.registryHash, calibrationReleaseHash: PHASE3D_CALIBRATION_RELEASE.releaseHash, calibrationTrustAnchorHash: PHASE3D_CALIBRATION_TRUST_ANCHOR.anchorHash, fullReportHash: "GENERATED_BY_RELEASE_SCRIPT", productionAuthorized: false as const, runtimeActivated: false as const, rankingAuthorized: false as const, eligibilityAuthorized: false as const, executionAuthorized: false as const });
