...
const MODEL_TARGETS = ["NONE", "EXPERIENCE_SUPPORT", "DIRECT_SPOT_PLANNING", "DIRECT_SPOT_AFFINITY", "FAMILIARITY", "CONTEXTUAL_CONCEPT_TASTE", "LONG_TERM_CONCEPT_TASTE", "AVERSION", "INTERACTION_ATTENTION", "CURRENT_DECISION_CONTEXT", "EXPLORATION_REQUEST", "SEMANTIC_CONFLICT", "PRIVACY_EXPORT"] as const;
...
const ProductEventSemanticsSchema = schema.object({
  ...
  independenceRule: schema.enum(["NEVER", "ONE_PER_RESOLVED_JOURNEY", "INDEPENDENT_QUALIFIED_ONLY", "TARGET_INHERITS"] as const), journeyPriority: schema.number({ min: 0, max: 100, integer: true }),
...
const PRODUCT_RULES: readonly ProductEventSemantics[] = [
  ...
  rule("SEARCH", base("Minimized structured search intent", { authorityRequirement: { mode: "ALL_OF", authorities: ["AUTHENTICATED_USER_ACTION", "SERVER_MINIMIZATION_SERVICE"] }, semanticTarget: "CONTEXTUAL_CONCEPT_TASTE", contextRequirement: "REQUIRED", independenceRule: "INDEPENDENT_QUALIFIED_ONLY", journeyPriority: 40, evidenceClass: "CONTEXTUAL_REPEATED", sufficiencyContribution: "SEARCH_CONTEXTUAL_PENDING_THRESHOLD", projectionEligibility: "POLICY_EVALUATION_ONLY", longTermTransfer: "NOT_CONFIGURED", prohibitedInterpretations: ["RAW_SEARCH_IS_PROFILE", "SINGLE_SEARCH_IS_LONG_TERM_TASTE"], reasonCodes: ["CURRENT_SEARCH_INTENT", "SEARCH_PROMOTION_NOT_CONFIGURED"] })),
  ...
  rule("QUICK_SKIP", base("Weak contextual skip", { semanticTarget: "DIRECT_SPOT_AFFINITY", contextRequirement: "REQUIRED", independenceRule: "ONE_PER_RESOLVED_JOURNEY", journeyPriority: 10, evidenceClass: "WEAK_CONTEXTUAL_NEGATIVE", sufficiencyContribution: "WEAK_PENDING_THRESHOLD", projectionEligibility: "POLICY_EVALUATION_ONLY", longTermTransfer: "FORBIDDEN", prohibitedInterpretations: ["SKIP_IS_GLOBAL_AVERSION", "SKIP_IS_CONCEPT_AVERSION"], reasonCodes: ["WEAK_SPOT_DECISION_CONTEXT_NEGATIVE", "SKIP_MATURITY_NOT_CONFIGURED"] })),
  ...
  rule("SPOT_NOT_FIT", base("Explicit spot does not fit current request", { authorityRequirement: { mode: "ALL_OF", authorities: ["AUTHENTICATED_USER_ACTION", "SERVER_VERIFIED_PRODUCT_STATE"] }, direction: "CONTEXTUAL_WEAK_NEGATIVE", semanticTarget: "DIRECT_SPOT_AFFINITY", contextRequirement: "REQUIRED", independenceRule: "ONE_PER_RESOLVED_JOURNEY", journeyPriority: 70, evidenceClass: "WEAK_CONTEXTUAL_NEGATIVE", sufficiencyContribution: "WEAK_PENDING_THRESHOLD", projectionEligibility: "POLICY_EVALUATION_ONLY", prohibitedInterpretations: ["SPOT_NOT_FIT_IS_GLOBAL_AVERSION"], reasonCodes: ["SPOT_DECISION_CONTEXT_NEGATIVE"] })),
...
export interface Phase3CReleaseTrustContext { getFounderDecisionRelease(recordId: string): unknown; getFounderDecisionTrustAnchor(anchorId: string): unknown; getProductPolicyRelease(policyId: string): unknown; getProductPolicyTrustAnchor(anchorId: string): unknown; }
export function verifyProductInterpretationPolicy(value: unknown, trust: Phase3CReleaseTrustContext): ProductInterpretationPolicy {
  ...
  if (!same(release, PRODUCT_POLICY_RELEASE_3C) || !same(anchor, PRODUCT_POLICY_TRUST_ANCHOR_3C) || release.acceptedPolicyHash !== policy.policyHash || anchor.acceptedPolicyHash !== policy.policyHash) throw new ContractValidationError("$.policyAuthority", "Product policy is not independently accepted");
  if (!same(policy, PRODUCT_INTERPRETATION_POLICY_3C)) throw new ContractValidationError("$", "unknown Product interpretation policy");
  return policy;
}

export const ProductPolicyObservationSchema = schema.object({ contractVersion: schema.literal(CONTRACT_VERSIONS.productPolicyObservation), recordId: identifier, subjectBindingHash: sha256, eventType: schema.enum(PRODUCT_POLICY_EVENT_TYPES), occurredAt: timestamp, active: schema.boolean(), sourceMode: schema.enum(["PHASE2_VERIFIED_ADAPTER", "SYNTHETIC_FUTURE_EVENT_FIXTURE"] as const), sourceEvidenceHash: sha256, sourceChainHash: sha256, authorityProofs: schema.array(identifier, { min: 1, max: 8 }), journeyId: schema.nullable(identifier), independenceEligible: schema.boolean(), spotId: schema.nullable(identifier), decisionId: schema.nullable(identifier), contextHash: schema.nullable(sha256), contextDimensions: schema.array(schema.enum(["COMPANY_TYPE", "DAY_PHASE", "COARSE_TIME_BUDGET", "REQUESTED_MOOD_CONCEPT"] as const), { max: 4 }), conceptIds: schema.array(identifier, { max: 16 }), worldAttribution: schema.enum(["NOT_APPLICABLE", "CERTAIN", "UNKNOWN", "CONFLICTING"] as const), experienceConfirmed: schema.boolean(), explicitOutcome: schema.enum(["POSITIVE", "NEGATIVE", "UNDECIDED", "NONE"] as const), explorationChoice: schema.nullable(schema.enum(["NEW", "FAMILIAR", "EITHER"] as const)), correctionTargetRecordId: schema.nullable(identifier), correctionTargetSourceEvidenceHash: schema.nullable(sha256), rawSensitiveDataIncluded: schema.literal(false), recordHash: sha256 });
...
function verifyObservation(value: unknown, subject: string, trust: Phase3CEvidenceTrustContext): ProductPolicyObservation {
  ...
  if ((row.eventType === "EXPLICIT_SATISFACTION" && row.explicitOutcome !== "POSITIVE") || (row.eventType === "EXPLICIT_DISSATISFACTION" && row.explicitOutcome !== "NEGATIVE") || (row.eventType === "EXPLICIT_UNDECIDED" && row.explicitOutcome !== "UNDECIDED")) throw new ContractValidationError("$.explicitOutcome", "explicit response direction mismatch");
  if (!["EXPLICIT_SATISFACTION", "EXPLICIT_DISSATISFACTION", "EXPLICIT_UNDECIDED"].includes(row.eventType) && row.explicitOutcome !== "NONE") throw new ContractValidationError("$.explicitOutcome", "non-outcome event cannot assert outcome");
...
export function deriveProductPolicyObservationFromCalibration(
  ...
  const body: Omit<ProductPolicyObservation, "recordHash"> = {
    ...
    explicitOutcome: evidence.explicitOutcome === "POSITIVE" ? "POSITIVE" : evidence.explicitOutcome === "NEGATIVE" ? "NEGATIVE" : "NONE",
...
export function evaluateProductInterpretationPolicy(input: ProductPolicyEvaluationInput, releaseTrust: Phase3CReleaseTrustContext, evidenceTrust: Phase3CEvidenceTrustContext): ProductPolicyEvaluation {
  ...
  const directedByJourney = new Map<string, ProductPolicyObservation>(); const priority = (row: ProductPolicyObservation) => row.eventType === "EXPLICIT_SATISFACTION" || row.eventType === "EXPLICIT_DISSATISFACTION" ? 100 : row.eventType === "SPOT_NOT_FIT" ? 70 : row.eventType === "QUICK_SKIP" ? 10 : -1;
  ...
  for (const [key, list] of searchGroups) interpretations.push(interpretation("CONTEXTUAL_CONCEPT_TASTE", `search:${key.split("|")[0]}`, "POSITIVE", list.length > 1 ? "TRANSITION_NOT_CONFIGURED" : "FIRST_HINT", list[0]!.contextHash, list.map(({ recordId }) => recordId), [list.length > 1 ? "REPEATED_INDEPENDENT_SEARCH_PATTERN" : "SINGLE_SEARCH_CURRENT_INTENT", "SEARCH_MATURITY_NOT_CONFIGURED", "NO_RAW_SEARCH"]));
  ...
  for (const [conceptId, spots] of conceptSpots) if (spots.size > 1) interpretations.push(interpretation("LONG_TERM_CONCEPT_TASTE", `concept:${conceptId}`, "POSITIVE", "TRANSITION_NOT_CONFIGURED", null, conceptEvidence.get(conceptId)!, ["MULTI_SPOT_CERTAIN_ATTRIBUTION", "MINIMUM_SPOT_THRESHOLD_NOT_CONFIGURED", "NOT_PROJECTABLE"]));
  ...
  const evaluationItems = interpretations.filter(({ interpretationId, maturity, kind }) => !conflictedIds.has(interpretationId) && !["TRANSITION_NOT_CONFIGURED"].includes(maturity) && kind !== "INTERACTION_ATTENTION").map(({ kind, targetKey, direction }) => ({ kind, targetKey, direction, uncertainty: "PRESERVED" as const }));
...
export function verifyProductPolicyState(value: unknown, releaseTrust: Phase3CReleaseTrustContext, evidenceTrust: Phase3CEvidenceTrustContext): ProductPolicyReducerState {
  ...
  const rebuilt = reducerState({ evaluationId: parsed.evaluation.evaluationId, subjectBindingHash: parsed.subjectBindingHash, lifecycle: "ACTIVE", purpose: parsed.evaluation.legalPrivacyExport.includesRequiredDerivedData ? "PRIVACY_EXPORT" : "POLICY_VALIDATION", observations: parsed.observations }, releaseTrust, evidenceTrust);
...
export function updateProductPolicyState(previousValue: unknown, delta: readonly unknown[], releaseTrust: Phase3CReleaseTrustContext, evidenceTrust: Phase3CEvidenceTrustContext): ProductPolicyReducerState {
  ...
  return reducerState({ evaluationId: previous.evaluation.evaluationId, subjectBindingHash: previous.subjectBindingHash, lifecycle: "ACTIVE", purpose: previous.evaluation.legalPrivacyExport.includesRequiredDerivedData ? "PRIVACY_EXPORT" : "POLICY_VALIDATION", observations: [...ledger.values()] }, releaseTrust, evidenceTrust);
}

export function createPhase3CRepositoryReleaseTrust(): Phase3CReleaseTrustContext {
  return { getFounderDecisionRelease: (recordId) => recordId === FOUNDER_DECISION_RECORD_3C.recordId ? FOUNDER_DECISION_RELEASE_3C : null, getFounderDecisionTrustAnchor: (anchorId) => anchorId === FOUNDER_DECISION_TRUST_ANCHOR_3C.anchorId ? FOUNDER_DECISION_TRUST_ANCHOR_3C : null, getProductPolicyRelease: (policyId) => policyId === PRODUCT_INTERPRETATION_POLICY_3C.policyId ? PRODUCT_POLICY_RELEASE_3C : null, getProductPolicyTrustAnchor: (anchorId) => anchorId === PRODUCT_POLICY_TRUST_ANCHOR_3C.anchorId ? PRODUCT_POLICY_TRUST_ANCHOR_3C : null };
}
