...
const registryBody = {
  ...
  registryVersion: "backyrd.user-intelligence.signal-semantics@3b-candidate-2",
...
export const CalibrationPolicySchema = schema.object({
  ...
  reducer: schema.object({ reducerVersion: schema.literal("backyrd.user-intelligence.calibration-reducer@3b.1"), releaseHash: sha256, productionRelease: schema.literal(false) }),
...
const reducerReleaseHash = contentHash("backyrd.user-intelligence.calibration-reducer@3b.1:fixture-only");
...
const SUFFICIENCY_STATES = ["NOTHING_KNOWN", "INSUFFICIENT_INDEPENDENCE", "INSUFFICIENT_DIVERSITY", "FIRST_HINTS", "REPEATED_ONE_SIDED", "CONFLICTING", "LIMITED_CALIBRATION_ONLY"] as const;
...
function sufficiency(interpretations: readonly CalibrationInterpretation[], used: readonly CalibrationEvidence[], policy: CalibrationPolicy): Infer<typeof SufficiencyMetricsSchema> {
  ...
  const groupedDirections = new Map<string, Set<string>>();
  for (const item of interpretations) { const key = `${item.dimension}|${item.key}|${item.contextHash ?? "none"}`; const directions = groupedDirections.get(key) ?? new Set<string>(); if (item.direction !== "UNKNOWN") directions.add(item.direction); groupedDirections.set(key, directions); }
  const semanticConflict = [...groupedDirections.values()].some((directions) => directions.has("POSITIVE") && directions.has("NEGATIVE"));
  ...
  const state = interpretations.length === 0 ? "NOTHING_KNOWN" : semanticConflict ? "CONFLICTING" : !independenceSufficient ? "INSUFFICIENT_INDEPENDENCE" : !diversitySufficient ? "INSUFFICIENT_DIVERSITY" : units.length >= policy.sufficiencyStrategy.limitedProjectionIndependentUnits ? "LIMITED_CALIBRATION_ONLY" : units.length > policy.sufficiencyStrategy.firstHintsIndependentUnits ? "REPEATED_ONE_SIDED" : "FIRST_HINTS";
...
function buildPolicyResult(scenario: CalibrationScenario, policy: CalibrationPolicy, policyTrust: CalibrationPolicyTrustContext, evidenceTrust: CalibrationEvidenceTrustContext): PolicyResult {
  ...
  const domainSufficiency = CALIBRATION_MODEL_DIMENSIONS.map((dimension) => {
    ...
    const perKey = new Map<string, Set<string>>(); for (const row of rows) { const values = perKey.get(row.key) ?? new Set<string>(); if (row.direction !== "UNKNOWN") values.add(row.direction); perKey.set(row.key, values); }
    const conflict = [...perKey.values()].some((values) => values.size === 2); const evidenceRows = used.filter(({ recordId }) => rows.some(({ evidenceRecordId }) => evidenceRecordId === recordId));
    ...
    const state = rows.length === 0 ? "NOTHING_KNOWN" as const : conflict ? "CONFLICTING" as const : !enoughIndependence ? "INSUFFICIENT_INDEPENDENCE" as const : !enoughDiversity ? "INSUFFICIENT_DIVERSITY" as const : units.length >= verifiedPolicy.sufficiencyStrategy.limitedProjectionIndependentUnits ? "LIMITED_CALIBRATION_ONLY" as const : units.length > verifiedPolicy.sufficiencyStrategy.firstHintsIndependentUnits ? "REPEATED_ONE_SIDED" as const : "FIRST_HINTS" as const;
