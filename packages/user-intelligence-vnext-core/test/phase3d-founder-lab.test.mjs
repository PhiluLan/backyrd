...
import {
  ...
  createPhase3DDecisionTaskBinding,
  ...
  verifyPhase3DFounderCalibrationDecisionRecord,
...
function run(items, lifecycle = "ACTIVE") {
  ...
  const options = { decisionTaskBindings: lifecycle === "ACTIVE" ? items.flatMap(({ decisionTaskBinding }) => decisionTaskBinding ? [decisionTaskBinding] : []) : [] };
  return { report: buildPhase3DFounderLabReport(input, releaseTrust, evidenceTrust, calibrationTrust, undefined, options), evidenceTrust, input, options };
}
...
const skip = (id, overrides = {}, decisionTaskId = "FIND_CAFE") => { const item = event(id, "QUICK_SKIP", { spotId: "spot-skip", decisionId: `decision-${id}`, ...overrides }); return { ...item, decisionTaskBinding: createPhase3DDecisionTaskBinding(item.observation, decisionTaskId) }; };
...
test("skip thresholds aggregate matching semantic targets while preserving decision provenance", () => {
  for (const [amount, reached] of [[2, []], [3, ["LEARNING_ORIENTED"]], [5, ["BALANCED", "LEARNING_ORIENTED"]], [8, ["CONSERVATIVE", "BALANCED", "LEARNING_ORIENTED"]]]) {
    const { report } = run(Array.from({ length: amount }, (_, index) => skip(`threshold-${amount}-${index + 1}`)));
    assert.deepEqual(report.candidateOutcomes.filter(({ maturedSkipTargets }) => maturedSkipTargets.length).map(({ candidateId }) => candidateId), reached);
    for (const outcome of report.candidateOutcomes) { const evaluation = outcome.skipMaturityEvaluations[0]; assert.equal(evaluation.independentJourneys, amount); assert.equal(new Set(evaluation.decisionExecutionIds).size, amount); assert.equal(evaluation.globalSpotAversion, false); assert.equal(evaluation.conceptAversion, false); assert.equal(evaluation.rankingAuthority, false); assert.equal(evaluation.eligibilityAuthority, false); }
  }
});
test("skip semantic target separates spot task and relevant context but not decision execution", () => {
  const evening = contentHash({ dayPhase: "evening", companyType: "alone" });
  const items = [skip("same-1"), skip("same-2"), skip("other-spot", { spotId: "other-spot" }), skip("other-context", { contextHash: evening }), skip("other-task", {}, "QUIET_MEAL")];
  const { report } = run(items); const learning = report.candidateOutcomes.find(({ candidateId }) => candidateId === "LEARNING_ORIENTED");
  assert.equal(learning.maturedSkipTargets.length, 0); assert.deepEqual(learning.skipMaturityEvaluations.map(({ independentJourneys }) => independentJourneys).sort(), [1, 1, 1, 2]);
});
test("inactive skip correction target and late delivery do not fabricate independence", () => {
  const items = [skip("active-1"), skip("active-2"), skip("inactive", { active: false }), skip("late", { occurredAt: "2025-01-01T08:00:00.000Z", journeyId: "journey-active-1" })];
  const { report } = run(items); const learning = report.candidateOutcomes.find(({ candidateId }) => candidateId === "LEARNING_ORIENTED"); assert.equal(learning.skipMaturityEvaluations[0].independentJourneys, 2); assert.equal(learning.maturedSkipTargets.length, 0);
});
test("skip task binding is mandatory exact and recursively hash-bound", () => {
  const item = skip("binding"); const evidenceTrust = createPhase3DLocalEvidenceTrust([item]); const input = { evaluationId: "binding-test", subjectBindingHash: subject, lifecycle: "ACTIVE", observations: [item.observation] };
  assert.throws(() => buildPhase3DFounderLabReport(input, releaseTrust, evidenceTrust, calibrationTrust));
  for (const field of ["decisionExecutionId", "journeyId", "spotId", "contextHash", "subjectBindingHash"]) { const forged = structuredClone(item.decisionTaskBinding); forged[field] = field.endsWith("Hash") ? contentHash(`forged-${field}`) : `forged-${field}`; forged.bindingHash = contentHash(Object.fromEntries(Object.entries(forged).filter(([key]) => key !== "bindingHash"))); assert.throws(() => buildPhase3DFounderLabReport(input, releaseTrust, evidenceTrust, calibrationTrust, undefined, { decisionTaskBindings: [forged] })); }
});
test("historical partial calibration record remains immutable and append-only linked", () => { const documented = JSON.parse(fs.readFileSync(new URL("../../../docs/user-intelligence-vnext/phase3d/PHASE3D_CALIBRATION_DECISIONS.json", import.meta.url), "utf8")); assert.equal(canonicalJson(documented), canonicalJson(PHASE3D_CALIBRATION_DECISION_RECORD)); assert.equal(PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD.previousRecordHash, PHASE3D_CALIBRATION_DECISION_RECORD.recordHash); });
test("all five Founder calibration selections are exact, non-production and documented", () => {
  const documented = JSON.parse(fs.readFileSync(new URL("../../../docs/user-intelligence-vnext/phase3d/PHASE3D_FOUNDER_CALIBRATION_DECISIONS.json", import.meta.url), "utf8"));
  assert.equal(canonicalJson(documented), canonicalJson(PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD)); assert.equal(verifyPhase3DFounderCalibrationDecisionRecord(documented).recordHash, documented.recordHash);
  assert.equal(documented.decisions.searchContextualMaturity.independentJourneys, 3); assert.equal(documented.decisions.searchLongTermPromotion.independentJourneys, 6); assert.equal(documented.decisions.skipMaturity.independentJourneys, 3); assert.equal(documented.decisions.conceptTasteMinimumDistinctSpots.distinctSpots, 3); assert.equal(documented.decisions.retention.strategy, "PURPOSE_TIERED_RETENTION"); assert.equal(documented.decisions.retention.durations, "NOT_CONFIGURED_PENDING_LEGAL_AND_CTO_APPROVAL");
  for (const decision of Object.values(documented.decisions)) assert.deepEqual(decision.status, { founder: "FOUNDER_SELECTED", calibration: "CALIBRATION_APPROVED", activation: "PRODUCT_ACTIVATION_NOT_AUTHORIZED" });
  for (const flag of ["productionAuthorized", "runtimeActivated", "rankingAuthorized", "eligibilityAuthorized", "shadowTrafficAuthorized"]) assert.equal(documented[flag], false);
});
test("Founder record manipulation and Production relabeling fail after complete rehash", () => { for (const mutate of [row => { row.decisions.skipMaturity.independentJourneys = 2; }, row => { row.productionAuthorized = true; }, row => { row.decisions.retention.durations = "30_DAYS"; }]) { const forged = structuredClone(PHASE3D_FOUNDER_CALIBRATION_DECISION_RECORD); mutate(forged); forged.recordHash = contentHash(Object.fromEntries(Object.entries(forged).filter(([key]) => key !== "recordHash"))); assert.throws(() => verifyPhase3DFounderCalibrationDecisionRecord(forged)); } });
test("machine traceability binds exactly all five Founder decisions", () => { const documented = JSON.parse(fs.readFileSync(new URL("../../../docs/user-intelligence-vnext/phase3d/PHASE3D_CALIBRATION_TRACEABILITY.json", import.meta.url), "utf8")); assert.equal(canonicalJson(documented), canonicalJson(PHASE3D_CALIBRATION_TRACEABILITY)); assert.equal(new Set(documented.rows.map(({ decisionId }) => decisionId)).size, 5); assert.ok(documented.rows.every(({ positiveTests, negativeTests }) => positiveTests.length && negativeTests.length)); for (const row of documented.rows) for (const testId of [...row.positiveTests, ...row.negativeTests]) assert.ok(PHASE3D_SCENARIO_IDS.includes(testId), `${row.decisionId}:${testId}`); });
test("Founder contextual search maturity is withheld at two and reached at three", () => { for (const amount of [2, 3]) { const { report } = run(Array.from({ length: amount }, (_, i) => search(`founder-context-${amount}-${i}`, { spotId: `spot-${i}` }))); assert.equal(report.founderSelectionOutcome.searchContextualTargets.length > 0, amount === 3); } });
test("Founder long-term search readiness is withheld at five and reached at six while Context stays bound", () => { for (const amount of [5, 6]) { const { report } = run(Array.from({ length: amount }, (_, i) => search(`founder-long-${amount}-${i}`, { spotId: `spot-${i}` }))); assert.equal(report.founderSelectionOutcome.searchLongTermTargets.length > 0, amount === 6); assert.equal(report.founderSelectionOutcome.productionProjectionAuthorized, false); } });
test("Founder concept threshold is withheld at two and reached at three distinct qualified spots", () => { for (const amount of [2, 3]) { const { report } = run(Array.from({ length: amount }, (_, i) => positive(`founder-concept-${amount}-${i}`, { spotId: `distinct-${i}` }))); assert.equal(report.founderSelectionOutcome.conceptTasteTargets.length > 0, amount === 3); } });
test("Founder purpose-tiered retention has no invented duration or Decision authority", () => { const { report } = run([]); assert.equal(report.founderSelectionOutcome.retentionStrategy, "PURPOSE_TIERED_RETENTION"); assert.equal(report.founderSelectionOutcome.retentionDurations, "NOT_CONFIGURED_PENDING_LEGAL_AND_CTO_APPROVAL"); assert.equal(report.founderSelectionOutcome.rankingAuthorized, false); assert.equal(report.founderSelectionOutcome.eligibilityAuthorized, false); });
...
test("report replay is byte-identical and recursively verified", () => { const result = run([search("replay-1"), positive("replay-2")]); const replay = buildPhase3DFounderLabReport(result.input, releaseTrust, result.evidenceTrust, calibrationTrust, undefined, result.options); assert.equal(canonicalJson(replay), canonicalJson(result.report)); assert.equal(verifyPhase3DFounderLabReport(result.report, result.input, releaseTrust, result.evidenceTrust, calibrationTrust, result.options).reportHash, result.report.reportHash); });
test("inner report manipulation fails after complete outer rehash", () => { const result = run([search("tamper")]); const forged = structuredClone(result.report); forged.candidateOutcomes[0].searchContextualTargets.push("contextual-search:forged"); forged.candidateOutcomes[0].outcomeHash = contentHash(Object.fromEntries(Object.entries(forged.candidateOutcomes[0]).filter(([key]) => key !== "outcomeHash"))); forged.reportHash = contentHash(Object.fromEntries(Object.entries(forged).filter(([key]) => key !== "reportHash"))); assert.throws(() => verifyPhase3DFounderLabReport(forged, result.input, releaseTrust, result.evidenceTrust, calibrationTrust, result.options)); });
test("scenario catalog covers every required search skip concept lifecycle integrity and browser family", () => { assert.equal(PHASE3D_SCENARIO_IDS.length, 64); assert.match(PHASE3D_SCENARIO_SET_HASH, /^[a-f0-9]{64}$/); for (const prefix of ["search-", "skip-", "concept-", "ui-"]) assert.ok(PHASE3D_SCENARIO_IDS.some((id) => id.startsWith(prefix))); });
