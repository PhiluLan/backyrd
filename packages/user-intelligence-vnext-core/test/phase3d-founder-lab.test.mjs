...
test("Founder-selected search calibrations are recorded without selecting skip maturity", () => { const documented = JSON.parse(fs.readFileSync(new URL("../../../docs/user-intelligence-vnext/phase3d/PHASE3D_CALIBRATION_DECISIONS.json", import.meta.url), "utf8")); assert.equal(canonicalJson(documented), canonicalJson(PHASE3D_CALIBRATION_DECISION_RECORD)); assert.equal(PHASE3D_CALIBRATION_DECISION_RECORD.decisions.searchContextualMaturity.independentJourneys, 3); assert.equal(PHASE3D_CALIBRATION_DECISION_RECORD.decisions.searchLongTermPromotion.independentJourneys, 6); assert.equal(PHASE3D_CALIBRATION_DECISION_RECORD.decisions.skipMaturity.status, "NOT_SELECTED"); assert.equal(PHASE3D_CALIBRATION_DECISION_RECORD.productionAuthorized, false); });
...
test("scenario catalog covers every required search skip concept lifecycle integrity and browser family", () => { assert.equal(PHASE3D_SCENARIO_IDS.length, 55); assert.match(PHASE3D_SCENARIO_SET_HASH, /^[a-f0-9]{64}$/); for (const prefix of ["search-", "skip-", "concept-", "ui-"]) assert.ok(PHASE3D_SCENARIO_IDS.some((id) => id.startsWith(prefix))); });
