...
export function validateDeterministicDecision({response,internal,decisionPackage,expectedUserId}) {
  ...
  if(response.spots.length!==Math.min(3,internal.fullOrder.length)||internal.fullOrder.length>decisionPackage.candidates.length||new Set(response.spots.map((x)=>x.spotId)).size!==response.spots.length)throw new Error("deterministic_response_result_count_invalid");
