...
export function buildDecisionInputPackage(source) {
  ...
  const candidates=eligibility.eligible.sort((a,b)=>a.retrievalPosition-b.retrievalPosition).map((candidate)=>{
    ...
    return {spotId:candidate.spotId,retrievalPosition:candidate.retrievalPosition,eligible:true,n4};
  });
  const candidateSetBody={decisionId:source.decision.id,candidates:candidates.map(({spotId,retrievalPosition,n4})=>({spotId,retrievalPosition,n4SnapshotHash:n4.snapshotHash}))};
  const body={
