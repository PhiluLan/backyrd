...
export interface WorldKnowledgeSnapshot {
  ...
  readonly spot: {
    ...
    readonly publicContact: { readonly publicEmail: string | null; readonly website: string | null; readonly phone: string | null; readonly instagram: string | null; readonly facebook: string | null; readonly linkedin: string | null; readonly tiktok: string | null };
  };
...
function readiness(resolution: ResolutionResult): readonly UseCaseReadiness[] {
  ...
  const priceKey = current.has("operation.price_level") ? "operation.price_level" : "operation.price_range"; const price = current.get(priceKey); rows.push(price ? row("PRICE", policy([priceKey], "PRICE"), ["PRICE_PRESENT", ...policyReasons([priceKey], "PRICE")]) : row("PRICE", "NOT_READY", ["PRICE_MISSING", ...policyReasons(["operation.price_level"], "PRICE")]));
...
export function buildWorldKnowledgeSnapshot(inputValue: unknown, acceptedPolicies: readonly Pick<SourcePolicy, "policyVersion" | "policyHash">[] = [UNCONFIGURED_SOURCE_POLICY], verificationContext?: AcceptedVerificationContext): WorldKnowledgeSnapshot {
  ...
  const body = {
    ...
    spot: { spotId: input.spotId, identity: { name: value<string>("identity.name") }, location: { addressLine1: value<string>("location.address_line1"), locality: value<string>("location.locality"), neighborhood: value<string>("location.neighborhood"), countryCode: value<string>("location.country_code"), latitude: value<number>("location.latitude"), longitude: value<number>("location.longitude"), timezone: value<string>("location.timezone") }, classification: { primaryCategory: value<PrimaryCategory>("classification.primary_category"), placeTypes: value<readonly PlaceType[]>("classification.place_types") ?? [] }, publicContact: { publicEmail: value<string>("contact.public_email"), website: value<string>("contact.website"), phone: value<string>("contact.phone"), instagram: value<string>("contact.instagram"), facebook: value<string>("contact.facebook"), linkedin: value<string>("contact.linkedin"), tiktok: value<string>("contact.tiktok") } },
...
export function parseWorldKnowledgeSnapshot(value: unknown, acceptedPolicies: readonly { readonly policyVersion: string; readonly policyHash: string }[] = [UNCONFIGURED_SOURCE_POLICY]): WorldKnowledgeSnapshot {
  ...
  const publicContact = object(required(spot, "publicContact", "$.spot"), "$.spot.publicContact", ["publicEmail", "website", "phone", "instagram", "facebook", "linkedin", "tiktok"]); for (const [field, key] of [["publicEmail", "public_email"], ["website", "website"], ["phone", "phone"], ["instagram", "instagram"], ["facebook", "facebook"], ["linkedin", "linkedin"], ["tiktok", "tiktok"]] as const) parseStructuralValue(`contact.${key}`, required(publicContact, field, "$.spot.publicContact"), `$.spot.publicContact.${field}`);
  ...
  if (ready.has("PRICE") && !["operation.price_level", "operation.price_range"].some((key) => entryAuthorized(key, "PRICE"))) throw new ContractValidationError("$.readiness", "price readiness lacks policy-authorized value");
