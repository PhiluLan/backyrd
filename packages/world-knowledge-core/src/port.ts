...
function readiness(resolution: ResolutionResult): readonly UseCaseReadiness[] {
  ...
  rows.push(has(...profileKeys, ...locationKeys, "classification.primary_category") ? row("DISCOVERY", "PARTIAL", ["DISCOVERY_CORE_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : row("DISCOVERY", "NOT_READY", ["DISCOVERY_CORE_MISSING", "SOURCE_POLICY_NOT_CONFIGURED"]));
  const hours = current.get("hours.regular"); rows.push(hours ? row("OPENING_HOURS_ELIGIBILITY", hours.trust === "ASSERTED" ? "NOT_READY" : "PARTIAL", [hours.trust === "ASSERTED" ? "OPENING_HOURS_ASSERTED_ONLY" : "OPENING_HOURS_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : row("OPENING_HOURS_ELIGIBILITY", "NOT_READY", ["OPENING_HOURS_MISSING", "SOURCE_POLICY_NOT_CONFIGURED"]));
  const price = current.get("operation.price_range"); rows.push(price ? row("PRICE", "PARTIAL", ["PRICE_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : row("PRICE", "NOT_READY", ["PRICE_MISSING", "SOURCE_POLICY_NOT_CONFIGURED"]));
  ...
  const hardCount = hardKeys.filter((key) => current.has(key)).length; rows.push(hardBlocking ? row("HARD_CONSTRAINTS", "NOT_READY", ["HARD_CONSTRAINT_CONFLICT_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : hardCount === hardKeys.length ? row("HARD_CONSTRAINTS", "PARTIAL", ["CORE_CONSTRAINTS_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : row("HARD_CONSTRAINTS", hardCount ? "PARTIAL" : "NOT_READY", [hardCount ? "CORE_CONSTRAINTS_INCOMPLETE" : "CORE_CONSTRAINTS_MISSING", "SOURCE_POLICY_NOT_CONFIGURED"]));
  const accessCount = accessKeys.filter((key) => current.has(key)).length; rows.push(accessCount === accessKeys.length ? row("ACCESSIBILITY", "PARTIAL", ["ACCESSIBILITY_COMPONENTS_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : row("ACCESSIBILITY", accessCount ? "PARTIAL" : "NOT_READY", [accessCount ? "ACCESSIBILITY_COMPONENTS_INCOMPLETE" : "ACCESSIBILITY_COMPONENTS_MISSING", "SOURCE_POLICY_NOT_CONFIGURED"]));
  rows.push(row("INTENT_MATCHING", "NOT_READY", ["CAPABILITY_INTENT_REGISTRY_NOT_CONFIGURED"]));
  const stale = resolution.resolved.some((item) => item.freshness !== "CURRENT"); const asserted = resolution.resolved.some((item) => item.trust === "ASSERTED"); rows.push(blocking ? row("TRUST_FRESHNESS", "NOT_READY", ["BLOCKING_CONFLICT_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : stale ? row("TRUST_FRESHNESS", "PARTIAL", ["STALE_OR_EXPIRED_KNOWLEDGE_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : asserted ? row("TRUST_FRESHNESS", "PARTIAL", ["ASSERTED_ONLY_KNOWLEDGE_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]) : row("TRUST_FRESHNESS", "PARTIAL", ["CURRENT_KNOWLEDGE_PRESENT", "SOURCE_POLICY_NOT_CONFIGURED"]));
  return rows;
}
...
export function parseBuildWorldKnowledgeInput(value: unknown): BuildWorldKnowledgeInput {
  ...
  const base = { contractVersion: WORLD_KNOWLEDGE_PORT_VERSION, spotId: identifier(required(input, "spotId"), "$.spotId"), resolution: parseResolutionResult(required(input, "resolution")) };
...
export function buildWorldKnowledgeSnapshot(inputValue: unknown): WorldKnowledgeSnapshot {
  const input = parseBuildWorldKnowledgeInput(inputValue);
...
export function parseWorldKnowledgeSnapshot(value: unknown): WorldKnowledgeSnapshot {
  ...
  array(required(input, "facts"), "$.facts").forEach((entry, index) => parseEntry(entry, `$.facts[${index}]`, "FACT")); array(required(input, "operationalRules"), "$.operationalRules").forEach((entry, index) => parseEntry(entry, `$.operationalRules[${index}]`, "OPERATIONAL_RULE")); array(required(input, "currentStates"), "$.currentStates").forEach((entry, index) => parseEntry(entry, `$.currentStates[${index}]`, "CURRENT_STATE"));
  ...
  for (const row of readinessRows.filter((item) => !["BASE_PROFILE", "CLASSIFICATION", "INTENT_MATCHING"].includes(item.useCase))) { if (row.state === "READY" || !row.reasonCodes.includes("SOURCE_POLICY_NOT_CONFIGURED")) throw new ContractValidationError("$.readiness", "trust-dependent readiness cannot be READY without configured source policy"); }
