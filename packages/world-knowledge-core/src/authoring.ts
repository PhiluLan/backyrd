...
export const AUTHORING_CATALOG_VERSION = "backyrd.world-knowledge.authoring-catalog@4b.1" as const;
export const FOUNDER_EXPORT_VERSION = "backyrd.world-knowledge.founder-export@2.0" as const;
export const FOUNDER_COHORT_VERSION = "backyrd.world-knowledge.founder-cohort@2.0" as const;
...
export type AuthoringControl = "TEXT" | "TEXTAREA" | "EMAIL" | "URL" | "PHONE" | "NUMBER" | "SINGLE_SELECT" | "MULTI_SELECT" | "YES_NO" | "INTEGER_RANGE" | "WEEKLY_SCHEDULE" | "KITCHEN_HOURS" | "SPECIAL_HOURS" | "RESERVATION_RULE" | "CONSUMPTION_RULE" | "PET_ACCESS_RULE" | "AGE_ACCESS_RULE" | "AGE_ACCESS_RULE_V2" | "CURRENT_STATE" | "ONSITE_OFFERINGS" | "VISIT_SITUATIONS" | "ATMOSPHERE_CONTEXTS" | "DAYPART_CONTEXTS";
...
export const AUTHORING_STEPS: readonly AuthoringStep[] = Object.freeze([
  ...
  { id: "context", order: 4, title: "Besuch und Atmosphäre", explanation: "Trenne den Hauptgrund für den Besuch von zusätzlichen Angeboten und beschreibe Situationen nur mit ihrem tatsächlichen Kontext.", primaryAction: "Besuchskontext übernehmen", attributeKeys: ["purpose.primary_visit", "offering.onsite", "context.visit_situations", "context.atmosphere", "context.typical_dayparts"] },
  { id: "price", order: 5, title: "Preise und Bezahlung", explanation: "Das Preislevel ist kategoriebezogen; es wird nicht in einen erfundenen Preis pro Person umgerechnet.", primaryAction: "Preisangaben übernehmen", attributeKeys: ["operation.price_level", "operation.payment_methods"] },
...
const germanValues: Readonly<Record<string, string>> = Object.freeze({
  ...
  EAT_DRINK: "Essen oder trinken", CULTURE_ARTS: "Kultur und Kunst", ENTERTAINMENT: "Unterhaltung", ACTIVITY_PLAY: "Aktivität und Spiel", SPORT_MOVEMENT: "Sport und Bewegung", NATURE_ANIMAL_EXPERIENCE: "Natur- oder Tiererlebnis", WELLNESS_RELAXATION: "Wellness und Entspannung", SHOPPING_MARKET: "Shopping oder Markt", OVERNIGHT_STAY: "Übernachten", COMMUNITY_SOCIAL: "Community und Begegnung", ATTRACTION_VISIT: "Attraktion besuchen", TEMPORARY_EVENT: "Temporäre Veranstaltung", OTHER: "Anderer Hauptgrund",
  KIOSK: "Kiosk", DRINKS: "Getränke", PICNIC: "Picknickmöglichkeit", HOTEL: "Unterkunft", SHOP: "Laden", KIDS_PLAY_AREA: "Kinder-Spielbereich", ALONE: "Allein", DATE_PAIR: "Date / zu zweit", FAMILY: "Familie", FRIENDS_GROUP: "Freunde / Gruppe", BUSINESS: "Geschäft / Business",
  QUIET: "Ruhig", LIVELY: "Lebendig", ROMANTIC: "Romantisch", COZY: "Gemütlich", CREATIVE: "Kreativ", RELAXED: "Entspannt", SOCIABLE: "Gesellig", ELEGANT: "Elegant", CASUAL: "Ungezwungen", FAMILY_FRIENDLY: "Familienfreundlich", BUSINESS_SUITABLE: "Geschäftlich geeignet",
  MORNING: "Vormittag", MIDDAY: "Mittag", AFTERNOON: "Nachmittag", EVENING: "Abend", NIGHT: "Nacht",
});
...
const help: Readonly<Record<string, string>> = Object.freeze({
  ...
  "purpose.primary_visit": "Der Hauptgrund, weshalb Menschen diesen Spot besuchen. Genau ein Primärzweck – Zusatzangebote ändern ihn nicht.",
  "offering.onsite": "Zusätzliche Angebote innerhalb des Spots. Ein Restaurant oder Café vor Ort macht den gesamten Spot nicht automatisch zu einem Restaurant- oder Café-Spot.",
  "context.visit_situations": "Typische Besuchssituationen gelten nur unter den angegebenen Bedingungen und sind keine universelle Eignungsaussage.",
  "context.atmosphere": "Atmosphäre kann sich je Tageszeit, Wochentag, Bereich oder Veranstaltung unterscheiden.",
  "context.typical_dayparts": "Beschreibt typische Nutzungszeiten. Ob der Spot tatsächlich geöffnet ist, bestimmen ausschließlich die Öffnungszeiten.",
});

const groupFor = (key: string): string => key === "purpose.primary_visit" ? "Hauptgrund" : key === "offering.onsite" ? "Zusätzliche Angebote" : key.startsWith("context.") ? "Situativer Kontext" : key.startsWith("contact.") ? "Öffentliche Kontakte" : key.startsWith("location.") ? "Standort" : key.startsWith("offering.") ? "Angebot" : key.startsWith("capacity.") ? "Kapazität" : key.startsWith("accessibility.") ? "Zugänglichkeit" : key.startsWith("hours.") ? "Zeiten" : key.startsWith("rule.") ? "Regeln" : key.startsWith("amenity.") ? "Ausstattung" : "Allgemein";
const controlFor = (valueType: string): AuthoringControl => ({ TEXT: "TEXT", EMAIL: "EMAIL", URL: "URL", PHONE: "PHONE", COUNTRY_CODE: "TEXT", IANA_TIMEZONE: "TEXT", DECIMAL: "NUMBER", INTEGER: "NUMBER", BOOLEAN: "YES_NO", ENUM: "SINGLE_SELECT", ENUM_SET: "MULTI_SELECT", INTEGER_RANGE: "INTEGER_RANGE", WEEKLY_SCHEDULE: "WEEKLY_SCHEDULE", SPECIAL_HOURS: "SPECIAL_HOURS", RESERVATION_RULE: "RESERVATION_RULE", CONSUMPTION_RULE: "CONSUMPTION_RULE", PET_ACCESS_RULE: "PET_ACCESS_RULE", AGE_ACCESS_RULE: "AGE_ACCESS_RULE", AGE_ACCESS_RULE_V2: "AGE_ACCESS_RULE_V2", CURRENT_STATE: "CURRENT_STATE", ONSITE_OFFERINGS: "ONSITE_OFFERINGS", VISIT_SITUATIONS: "VISIT_SITUATIONS", ATMOSPHERE_CONTEXTS: "ATMOSPHERE_CONTEXTS", DAYPART_CONTEXTS: "DAYPART_CONTEXTS" } as Partial<Record<string, AuthoringControl>>)[valueType] ?? "TEXT";
...
export const AUTHORING_FIELDS: readonly AuthoringField[] = Object.freeze(AUTHORING_STEPS.flatMap((step) => step.attributeKeys.map((attributeKey) => {
  ...
  return {
    ...
    allowedValues: (attributeKey === "classification.place_types" ? PLACE_TYPE_AUTHORING_OPTIONS
      ...
      : attributeKey === "offering.onsite" ? ONSITE_OFFERING_KINDS.map((value) => ({ value, label: germanValues[value] ?? value, state: "CANONICAL" as const }))
      : attributeKey === "context.visit_situations" ? VISIT_SITUATIONS.map((value) => ({ value, label: germanValues[value] ?? value, state: "CANONICAL" as const }))
      : attributeKey === "context.atmosphere" ? ATMOSPHERE_VALUES.map((value) => ({ value, label: germanValues[value] ?? value, state: "CANONICAL" as const }))
      : attributeKey === "context.typical_dayparts" ? DAYPARTS.map((value) => ({ value, label: germanValues[value] ?? value, state: "CANONICAL" as const }))
...
const correctionFor = (valueType: ValueType): string => ({
  ...
  ONSITE_OFFERINGS: "Bitte wähle Angebot und Beziehung zum Spot. Angebote in der Nähe gehören nicht hierher.",
  VISIT_SITUATIONS: "Bitte wähle eine Besuchssituation und prüfe die optionalen Bedingungen.",
  ATMOSPHERE_CONTEXTS: "Bitte wähle eine Atmosphäre und gib abweichende Tageszeiten oder Bereiche als Bedingung an.",
  DAYPART_CONTEXTS: "Bitte wähle typische Nutzungszeiten; Öffnungszeiten werden separat erfasst.",
...
export interface FounderSpotExport {
  ...
  readonly contextualHandoff: WorldKnowledgeContextHandoff;
...
export function createFounderSpotExport(input: { readonly manifestHash: string; readonly snapshot: unknown }): FounderSpotExport {
  ...
  const contextualHandoff = createWorldKnowledgeContextHandoff(snapshot, [ACCEPTED_SOURCE_POLICY], snapshot.resolvedAt);
  const body = { contractVersion: FOUNDER_EXPORT_VERSION, scope: FOUNDER_EVALUATION_SCOPE, spotId: snapshot.spot.spotId, registryVersion: REGISTRY_VERSION, registryHash: REGISTRY_HASH, policyVersion: snapshot.sourcePolicyVersion, policyHash: snapshot.sourcePolicyHash, manifestHash: input.manifestHash, snapshot, contextualHandoff };
...
export interface FounderWorldCohortManifest {
  ...
  readonly policyVersion: string;
  readonly policyHash: string;
  readonly spots: readonly { readonly spotId: string; readonly manifestHash: string; readonly snapshotHash: string; readonly contextHandoffHash: string }[];
...
export function createFounderWorldCohortManifest(input: { readonly cohortId: string; readonly frozenAt: string; readonly exports: readonly FounderSpotExport[] }): FounderWorldCohortManifest {
  ...
  if (new Set(input.exports.map((item) => item.spotId)).size !== input.exports.length) throw new Error("duplicate_spot_in_cohort");
  const spots = canonicalSort(input.exports.map((item) => ({ spotId: item.spotId, manifestHash: item.manifestHash, snapshotHash: item.snapshot.snapshotHash, contextHandoffHash: item.contextualHandoff.handoffHash })), (item) => item.spotId);
