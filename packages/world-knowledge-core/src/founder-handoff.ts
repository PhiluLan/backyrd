...
function parseContextHandoff(value: unknown, expectedSpotId: string, expectedHash: string, path: string): FounderContextShadowHandoff {
  ...
  const rawEntries = input.entries as Record<string, unknown>; const entries = Object.fromEntries(Object.entries(rawEntries).map(([key, entry]) => {
    if (!isContextKey(key)) throw new Error(`${path}.entries_unknown_key`); const parsed = parseFact(entry, `${path}.entries.${key}`); if (parsed.key !== key) throw new Error(`${path}.entries_key_mismatch`); return [key, parsed];
  ...
  if (new Set(absentKeys).size !== absentKeys.length || new Set(explicitUnknowns).size !== explicitUnknowns.length || absentKeys.some((key) => explicitUnknowns.includes(key) || key in entries) || explicitUnknowns.some((key) => key in entries)) throw new Error(`${path}_knowledge_state_overlap`);
