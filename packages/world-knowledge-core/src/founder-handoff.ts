...
export const FOUNDER_SHADOW_COHORT_VERSION = "backyrd.world-knowledge.founder-cohort-shadow@2.0" as const;
...
export interface FounderShadowCohortManifest {
  ...
  readonly registryVersion: typeof REGISTRY_VERSION;
  readonly policyVersion: typeof ACCEPTED_POLICY_VERSION;
  readonly spots: readonly { readonly spotId: string; readonly manifestHash: string; readonly resolutionHash: string; readonly inputHash: string; readonly snapshotHash: string }[];
...
export interface FounderWorldCohortHandoff {
  ...
  readonly compatibilityMode: "REGISTRY_1_1_TO_2_0_EXPLICIT_EVALUATION_ONLY";
...
const expectedExclusions = ["ADMIN_NOTES", "OWNER_TIER", "PAYMENT", "PRIVATE_ACTOR_IDS", "PRIVATE_SOURCE_REFERENCES", "RAW_AI_OUTPUTS", "SUBSCRIPTION"] as const;
const decisionAllowedPrefixes = ["classification.", "offering.", "hours.", "capacity.", "rule.", "operation.", "amenity.", "accessibility.", "state."] as const;
const isDecisionAllowedKey = (key: string) => key === "identity.name" || key === "location.locality" || key === "location.neighborhood" || key === "location.country_code" || key === "location.timezone" || decisionAllowedPrefixes.some((prefix) => key.startsWith(prefix)) && key !== "operation.payment_methods";
...
function parseManifest(value: unknown): FounderShadowCohortManifest {
  ...
  const spots = list(input.spots, "founder_manifest.spots", 1, 40).map((item, index) => { const row = record(item, `founder_manifest.spots[${index}]`, ["spotId", "manifestHash", "resolutionHash", "inputHash", "snapshotHash"]); return { spotId: identifier(row.spotId, `founder_manifest.spots[${index}].spotId`), manifestHash: hash(row.manifestHash, `founder_manifest.spots[${index}].manifestHash`), resolutionHash: hash(row.resolutionHash, `founder_manifest.spots[${index}].resolutionHash`), inputHash: hash(row.inputHash, `founder_manifest.spots[${index}].inputHash`), snapshotHash: hash(row.snapshotHash, `founder_manifest.spots[${index}].snapshotHash`) }; });
  if (new Set(spots.map((item) => item.spotId)).size !== spots.length) throw new Error("founder_manifest_duplicate_spot");
  const exclusions = list(input.exclusions, "founder_manifest.exclusions", expectedExclusions.length, expectedExclusions.length).map((item, index) => text(item, `founder_manifest.exclusions[${index}]`)).sort();
  if (canonicalJson(exclusions) !== canonicalJson([...expectedExclusions].sort())) throw new Error("founder_manifest_exclusions_mismatch");
  return { contractVersion: FOUNDER_SHADOW_COHORT_VERSION, scope: "FOUNDER_EVALUATION_ONLY", cohortId: identifier(input.cohortId, "founder_manifest.cohortId"), registryVersion: REGISTRY_VERSION, policyVersion: ACCEPTED_POLICY_VERSION, spots, exclusions, cohortHash: hash(input.cohortHash, "founder_manifest.cohortHash") };
}
...
export function createFounderWorldCohortHandoff(input: { readonly manifest: unknown; readonly spotDetails: readonly { readonly spotId: string; readonly fallbackName: string; readonly manifest: { readonly manifestHash: string; readonly worldSnapshot: unknown } }[] }): FounderWorldCohortHandoff {
  ...
  const body = { contractVersion: FOUNDER_COHORT_HANDOFF_VERSION, scope: "FOUNDER_EVALUATION_ONLY" as const, source: "WORLD_KNOWLEDGE_FOUNDER_EXPORT" as const, manifest, manifestContentHash: hashBody(manifest as unknown as Record<string, unknown>, []), registryHash: REGISTRY_HASH, policyHash: ACCEPTED_SOURCE_POLICY.policyHash, compatibilityMode: "REGISTRY_1_1_TO_2_0_EXPLICIT_EVALUATION_ONLY" as const, spots, exportedAt, productionAuthorized: false as const, productQualityClaim: false as const };
...
export function parseFounderWorldCohortHandoff(value: unknown): FounderWorldCohortHandoff {
  ...
  const parsed = { contractVersion: FOUNDER_COHORT_HANDOFF_VERSION, scope: "FOUNDER_EVALUATION_ONLY" as const, source: "WORLD_KNOWLEDGE_FOUNDER_EXPORT" as const, manifest, manifestContentHash, registryHash: REGISTRY_HASH, policyHash: ACCEPTED_SOURCE_POLICY.policyHash, compatibilityMode: "REGISTRY_1_1_TO_2_0_EXPLICIT_EVALUATION_ONLY" as const, spots: rows, exportedAt: new Date(text(input.exportedAt, "founder_handoff.exportedAt")).toISOString(), productionAuthorized: false as const, productQualityClaim: false as const, handoffHash: hash(input.handoffHash, "founder_handoff.handoffHash") };
